...
scraped_at: "2026-04-20T18:19:47.662Z"
images: 3
---
...
In Vault Core 5, the Parameter system supports a similar set of types (also known as shapes, or constraints) to the pre-Vault Core 5.0 Parameters, but some have been improved, or in some cases consolidated. This is to make it easier to write and read Smart Contracts by requiring fewer Parameters overall, and making them more closely match the required usages.
...
-   All existing Global and Instance Parameters will have been migrated onto the new Parameter system, and are available via the new `v1/parameters` and `v1/parameter-values` endpoints (ready for when you [switch to the Core API Parameters resource](/vault-core/5-8/EN/reference/parameters/switching_to_core_api_parameters))
    
-   All existing APIs used to define Global and Instance Parameters (and their values) will continue to work until their official removal, but any Parameters and values defined via these APIs will also be automatically created in the Parameters resource, and are available via the Parameter API endpoints
...
Global and Instance Parameters are mapped according to their name and shape, regardless of their level.
...
-   Both a matching name and shape, only a single mapped Parameter will be created (using the name as its ID)
    
-   The same name but a different shape, one new Parameter (type) will be created for each shape; for one of these shapes the new Parameter’s ID will match the name, but for the others the name will be suffixed with an integer
    
-   A different name and shape, one new Parameter will be created for each (using the name as its ID)
...
The following diagram illustrates the Vault Core 5 mapping of Global and Instance Parameters to the new Parameters (Smart Contracts are included for context):

-   **Global Parameter 1** and **Instance Parameter 1** and **2** have the same shape and name, so their name is mapped to only one Parameter ID in the new resource (A)
    
-   **Global Parameter 2** and **Instance Parameter 3** have the same name but a different shape to the previous Parameters, so a new Parameter is created, and the ID is suffixed with an integer (B)
    
-   **Global Parameter 3** and **Instance Parameter 4** have a different name to the previous Parameters, so the new Parameter is created with its name as the ID (C)
    

![Vault\_Core\_5\_0\_\_diagrams\_-\_Param\_mapping.svg](_assets/uuid-783f26ea-12a6-676b-325c-b097ca39a78_vaultcor.svg)

#### [](#parameter_value_mapping "Copy link to heading")Parameter Value mapping

-   Each Global Parameter Value will display on the Parameter Values resource as a Parameter Value with an owner set to `global: true`
    
-   Each Instance Parameter Value will display on the Parameter Values resource as a Parameter Value owned by a specific Account (`account_id`)
