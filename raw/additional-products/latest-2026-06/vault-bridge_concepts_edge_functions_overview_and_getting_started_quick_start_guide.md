---
source_url: "https://vault-portal.thoughtmachine.net/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/overview_and_getting_started/quick_start_guide"
title: "Quick start guide"
scraped_at: "2026-06-17T05:14:41.303Z"
images: 0
---
...
Each Edge Function contains Python code defined as a single Python module which can interact with Vault services and third-party REST APIs. To execute an existing Edge Function, your application must make an HTTP call to the Vault Bridge `execute` endpoint. This triggers the execution of the corresponding Python code, which orchestrates HTTP calls to the target services.

The SDK consists of the `edge_api` and `vc_api` libraries. These two libraries contain all the functionality necessary to structure Edge Functions to call Vault services in an intuitive way.

The Edge Function can use the SDK to call a subset of [Vault Core APIs](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/allowed_endpoints). The HTTP call to execute the Edge Function returns a response that includes the output of the Python execution.

In order to define an Edge Function, you need to provide a Python module with a module-level function that has an `entry_point` decorator. This function acts as the entry point for the execution. Upon invocation, the Edge Function is provided with `ExecutionContext` and `Request` objects as well as any \`HTTPClient\`s which are defined.

lightbulb

If you are visiting this tutorial for the first time, take a look at [this diagram for an overview of key Edge Function resources](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_an_edge_function#key_edge_function_resources), and how Edge Functions and Edge Function Versions relate to each other.
...
For more information about the use of [Pydantic](https://docs.pydantic.dev/) objects, the functionality and examples, see the [How to write an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_an_edge_function/) tutorial.
...
-   The `edge_api` library offers a robust framework for authoring and testing Edge Functions, featuring the `HTTPClient` for making downstream requests.
    

For detailed instructions about installing the SDK, see the [Edge Functions installation guide](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/overview_and_getting_started/installation_guide).
...
Authentication for any downstream calls made by the Edge Function (such as to Vault Core or Vault Payments) is handled automatically by the Vault Bridge platform using [Integration Configs and Credentials](/additional-product-offerings/latest/EN/vault-bridge/concepts/integrations). This allows the platform to use the pre-configured, least-privilege credentials associated with the Edge Function to authenticate its requests.

The default OPA policy for the Edge Functions API follows the standard Bridge patterns. For more information on policy management, see the [Static Policies](/vault-core/latest/EN/api/overview#static_policies_2) documentation.

To learn more about setting up Integration Configs and Credentials, see the [Edge Functions installation guide](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/overview_and_getting_started/installation_guide).
...
You can find an example Edge Function for opening an Account, which you can edit. Thought Machine recommends reading [How to write an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_an_edge_function) for further information on the constituent parts of an Edge Function. However, the following example provides you with a good starting point.
...
You must execute an Edge Function via a POST request to the HTTP endpoint. The URL of the endpoint must contain the ID of the Edge Function that you want to execute. The system always executes the active version of the Edge Function by default.

In the event that the Edge Function execution fails, it is possible for you to [manually retry a failed Edge Function execution request](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_retry_an_execution) if you have included an idempotency key in the first Edge Function execution request.

Here, you can get a quick overview of the call to execute an Edge Function. For a more complete guide to executing Edge Functions and retrying calls, see [How to execute an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_execute_an_edge_function) and [How to retry an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_retry_an_execution).

### [](#get_a_list_of_edge_functions_listedgefunctions "Copy link to heading")Get a list of Edge Functions: ListEdgeFunctions

Permission scopes: `bridge:read`, `bridge.edge_functions:read`
...
*Optional*. The version tag of the Edge Function Version (EFV) that you wish to execute. The default behaviour if you do not pass this parameter is to execute the EFV that is set as the `current_version` of the Edge Function. For more information, see: [Executions default to the current Edge Function version](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_execute_an_edge_function#executions_default_to_the_current_edge_function_version)
...
For more information about executing and retrying Edge Functions, see [How to execute an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_execute_an_edge_function) and [How to retry an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_retry_an_execution).
...
The unique identifier for the `EdgeFunctionExecution`
...
In the event that the Edge Function execution fails, it is possible for you to [manually retry a failed Edge Function execution request](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_retry_an_execution) using the idempotency key from the first Edge Function execution request.

## [](#handling_errors_and_customising_error_messages "Copy link to heading")Handling errors and customising error messages

When using the `vc_api` library to make calls to Vault Core, if a request receives a response other than HTTP `200`, the library automatically converts it to an error response.
...
-   [How to write an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_an_edge_function/) - learn about implementing error handling, logging, and customising error messages to your Edge Functions code
    
-   [Monitoring](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/overview_and_getting_started/observability) - learn about monitoring Edge Functions and advice about errors
    
-   [Troubleshooting Edge Functions](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/troubleshooting) - get advice on how to diagnose any issues with Edge Functions
    
-   [Edge Functions API reference](/additional-product-offerings/latest/EN/vault-bridge/api/bridge_api#_edge_functions)
    

## [](#test_an_edge_function "Copy link to heading")Test an Edge Function

You must first [compose an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_an_edge_function) before you can test it. You can then run unit tests against it using the unit testing utilities that are available in the SDK.
...
The `vc_api.test` module includes the `@patch_core_api_client` decorator. This provides a mock Core API client object that allows you to add mock responses and mock errors to specific mock requests made on the Core API Client.

To learn more about how to test Edge Functions, see the following tutorials:

-   [How to write a unit test for an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_a_unit_test_for_an_edge_function) - learn about writing the tests yourself
    
-   [How to test an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_test_an_edge_function) - get an overview of the testing flow and advise, from develop your Edge Function code and tests locally to testing and deploying it in your environments