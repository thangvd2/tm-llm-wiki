---
source_url: "https://vault-portal.thoughtmachine.net/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_test_an_edge_function"
title: "How to test an Edge Function"
scraped_at: "2026-06-17T15:53:49.455Z"
images: 0
---
...
The [How to write a unit test for an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_a_unit_test_for_an_edge_function) tutorial provides information about writing the tests yourself. This include instructions on mocking the Core API, using the `@patch_core_api_client` decorator, if it is used within your Edge Functions.
...
The platform always executes the active version of the Edge Function by default if you do not specify a version on execution. It is possible for you to specify a particular version of an Edge Function to execute. For example, to test a particular version and validate its behaviour before making it the active version. However, Thought Machine does not recommend using this pattern to retain a specific version outside of this use case. This is to ensure good Edge Function and execution management hygiene. You should only use this functionality to test a version in your environment. For example, before you set it as the active version in a production environment.
...
1.  Compose your Edge Function source code for its initial or a new Edge Function Version - for a guide, see [How to write an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_an_edge_function).
    
2.  Compose a unit test - see [How to write a unit test for an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_a_unit_test_for_an_edge_function).
...
1.  Upload the Edge Function to your pre-production environment - see [How to upload an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_upload_an_edge_function) and the [Configuration Layer Utility Guide](/vault-core/latest/EN/environment_and_installation/infrastructure_docs/configuration_layer_utility_user_guide).
...
3.  Make the Edge Function Version the active version in your pre-production environment.
    
4.  Execute the Edge Function in your pre-production environment with the active version.
...
3.  Make the Edge Function Version the active version in the production environment.
    
4.  Execute the Edge Function in the production environment with the tested code in the Edge Function Version set as the active version.
...
You must first [compose an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_an_edge_function) before you can test it. You can then run unit tests against it using the unit testing utilities that are available in the Edge Functions SDK.

When you are ready to test it in your pre-production environment, you must [upload the Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_upload_an_edge_function) to make it available as a resource in your pre-production and production environments.

This allows you to execute the Edge Function by making a call to the and the [`ExecuteEdgeFunction` endpoint](/additional-product-offerings/latest/EN/vault-bridge/api/bridge_api#edge_functions), with the option to specify a particular Edge Function Version.
...
For guidance on how to write a unit test for an Edge Function, see: [How to write a unit test for an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_a_unit_test_for_an_edge_function)
...
For more information, see the [Edge Functions installation guide](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/overview_and_getting_started).
...
The following example is of a unit test file that is suitable to test the example simple Edge Function, which creates a Customer, in the [Quick start guide](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/overview_and_getting_started/quick_start_guide).
...
To provide some context to these and assist writers of tests, see [How to write a unit test for an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_write_a_unit_test_for_an_edge_function).
...
Once you are satisfied with the Edge Function passing the unit tests, you can [upload the Edge Function to your pre-production environment](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_upload_an_edge_function) to make it available to test.
...
You can test the Edge Function Version before you make it the 'active version' by specifying the version in the execution request to the `edge_function_version_tag`. For example, if you have created a new Edge Function Version subresource with updated the source code for an existing Edge Function resource.
...
For more information about making execution requests, see the Edge Functions Reference section and [How to execute an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_execute_an_edge_function).

### [](#make_the_edge_function_version_the_active_version_in_your_pre_production_environment "Copy link to heading")Make the Edge Function Version the active version in your pre-production environment

Once you are satisfied that the Edge Function executes as you expect with the source code you are testing, you can make it the active version in your pre-production environment.

To do this, update the Edge Function - see [How to update an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_update_an_edge_function).

### [](#execute_the_edge_function_in_your_pre_production_environment_with_the_active_version "Copy link to heading")Execute the Edge Function in your pre-production environment with the active version

Once you have updated the Edge Function resource so that this Edge Function Version is the active version, you can test it again without specifying the version tag.

The platform will then always execute the active version of the Edge Function by default. For guidance on making execution requests, refer to the example below, the Reference section, and [How to execute an Edge Function](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/how_to_execute_an_edge_function).

#### [](#example_request_to_test_the_active_version_of_an_edge_function "Copy link to heading")Example request to test the active version of an Edge Function:
...
You can observe the general health of execution requests using the observability tools included with Edge Functions, such as the Grafana dashboards. For more information, see [Monitoring](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/overview_and_getting_started/observability).
...
2.  If this is a new version for an existing Edge Function, update the parent Edge Function to use the new Edge Function Version as the active version.
    
3.  Once you have uploaded the new Edge Function Version code, the only way to test it is by executing the Edge Function. You can do this either by executing the active version or explicitly using the `edge_function_version_tag` parameter to specify a version.
    
    warning
    
    Thought Machine strongly recommends that you do NOT test Edge Functions that, on execution, result in creating data within Vault Core on a live production environment as part of the test. For example, test accounts created by the execution. This is because it could result in a negative business impact.
    

For monitoring and debugging issues in a production environment, see [Monitoring](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/overview_and_getting_started/observability).

chat\_bubble

If you experience any issues with testing and Edge Functions, refer to [How to troubleshoot Edge Functions](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/edge_functions_tutorials/troubleshooting). If you require further assistance, contact Thought Machine Support.