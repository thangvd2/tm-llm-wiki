---
source_url: "https://vault-portal.thoughtmachine.net/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/managing_triggers"
title: "Edge Functions Trigger Management"
scraped_at: "2026-06-17T15:54:01.592Z"
images: 0
---

# Edge Functions Trigger Management
...
When a Trigger is made `ACTIVE` for the first time, it will start listening for the most recent events on the source event Kafka topic (subject to [Lag in response to requested changes](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/managing_triggers#lag)). This means it will ignore all historical events within that topic.

See also [Trigger reactivation](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/managing_triggers#reactivation).

## [](#trigger_deactivation "Copy link to heading")Trigger deactivation

When a Trigger is made `INACTIVE`, it will stop listening to source events (subject to [Lag in response to requested changes](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/managing_triggers#lag)). If the Trigger was previously `ACTIVE`, then it will remember the point where it stopped listening to source events. Any Trigger Operations that exist up to this point will continue to be executed, even if the Trigger is deactivated before they can be executed.
...
If a Trigger was previously `ACTIVE`, and it was made `INACTIVE`, then it will have remembered the last source events that it processed. If it is then made `ACTIVE` again (subject to [Lag in response to requested changes](/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/managing_triggers#lag)), it will continue processing source events from that point forwards, including any backlog of events that has accumulated in the intervening time.
