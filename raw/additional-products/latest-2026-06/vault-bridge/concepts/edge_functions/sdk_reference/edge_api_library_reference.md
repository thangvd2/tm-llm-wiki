---
source_url: "https://vault-portal.thoughtmachine.net/additional-product-offerings/latest/EN/vault-bridge/concepts/edge_functions/sdk_reference/edge_api_library_reference"
title: "Library reference: edge_api"
scraped_at: "2026-06-17T15:53:35.134Z"
images: 0
---
...
## [](#basehttperror "Copy link to heading")BaseHTTPError

*type: Class*

Base class for errors raised relating to making HTTP requests from an edge function.
...
## [](#externalsystemtype "Copy link to heading")ExternalSystemType

*type: Enum*

Type of External System created using the Bridge API.

```
:attr CORE: A Vault Core system.
:attr THIRD\_PARTY: A third party external HTTP system.
:attr PAYMENTS: A Vault Payments system.
```
...
## [](#httpclient "Copy link to heading")HTTPClient

*type: Class*

Protocol representing an HTTP client.

Signature of the constructor for this class:

## [](#httpclientconfig "Copy link to heading")HTTPClientConfig

*type: Class*

Describes the configuration of a HTTPClient.

```
vault\_api field is only supported when integration\_config.external\_system\_type is ExternalSystemType.CORE.
- If the integration\_config external\_system\_type is ExternalSystemType.CORE and this field is None, vault\_api defaults to VaultAPI.CORE.
- If the integration\_config external\_system\_type is not ExternalSystemType.CORE, vault\_api field must remain None.
```
...
`class 'edge_api._integrations.IntegrationConfig'`

 | 

The Integration Config used by the HTTPClient.
...
## [](#httpconnectionerror "Copy link to heading")HTTPConnectionError
...
## [](#httperror "Copy link to heading")HTTPError
...
HTTPMethod of the HTTP request.
...
Describes an Integration Config which has been created using the Bridge API.
...
Type of the External System linked to the Integration Config.
...
\[IMPORTANT\]
====
Do not include Personally Identifiable Information (PII) or sensitive personal data
in log messages. Any information passed to the logger is output directly to the system logs.
====
```
