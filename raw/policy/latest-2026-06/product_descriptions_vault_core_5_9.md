---
source_url: "https://vault-portal.thoughtmachine.net/policy/latest/EN/product_descriptions/vault_core_5_9"
title: "Vault Core Version 5.9 Product Description"
scraped_at: "2026-06-17T05:17:41.305Z"
images: 0
---

# Vault Core Version 5.9 Product Description

[Download PDF](/policy/latest/EN/resources/vault_core_5_9.pdf)

## [](#overview "Copy link to heading")1\. Overview

This product description describes the following for Vault Core Version 5.9
...
Details on the specific components of the Vault Core product and information about the APIs are available on Thought Machine’s Vault client portal website (“Client Portal”). Updated and/or amended versions of this product description will be published on the Client Portal. This product description sets out the Vault Core "Standard" offering. However, this product description may also include extension features and functionality (which will be expressly called out using the signpost: "Extension") which do not form part of the "Standard" offering and are only included where that Extension has been contractually agreed to be included in a signed agreement between Thought Machine and the Client.
...
-   Use of the current financial product logic, or the historic financial product logic in place when each event was first processed
...
#### [](#saml_authentication "Copy link to heading")5.17.1. SAML Authentication

Vault Core allows clients to set up SAML for authentication and authorisation for Core Apps. These are managed through a combination Roles (version 1) and Permissions resources

##### [](#define_and_apply_permissions "Copy link to heading")5.17.1.1. Define and apply Permissions
...
##### [](#define_and_apply_operational_roles "Copy link to heading")5.17.1.2. Define and apply operational Roles

Roles (version 1) in Vault Core are groups of one or more Permissions. Clients can use them to assign access rights to their employees. Permissions are assigned to a Role (version 1) to define the access granted to any employee with that Role.

#### [](#oidc_authentication "Copy link to heading")5.17.2. OIDC Authentication

Vault Core allows clients to set up OIDC for authentication and authorisation into the Vault Accounts, Vault Jobs, DLQ Inspector, Processing Group, and Usage Monitor applications. These are managed through a Roles (version 2) resource.

##### [](#define_and_apply_operational_roles_2 "Copy link to heading")5.17.2.1. Define and apply operational Roles

Roles (version 2) in Vault Core are groups of one or more Privileges. Clients can use them to assign access rights to their employees.

##### [](#define_and_apply_product_based_restrictions "Copy link to heading")5.17.2.2. Define and apply Product Based Restrictions

Roles (version 2) can also contain Product Based Restrictions. Clients using these roles (version 2) will have these restrictions applied while doing supported actions when using the Vault Accounts App. Product Based Restrictions is an Extension.
...
Clients running Vault Core in AWS have the option to configure Vault Core to authenticate against a database using Role-based Access Control (RBAC) with AWS IAM credentials. Both RDS and Aurora versions of Postgres are supported by the APIs provided by AWS. IAM role names can be configured with a customisable prefix.
...
-   Inflight messages on inbound public Kafka topics may be dropped before they are able to be consumed or processed by Vault Core. These topics must be restored manually by the Client (Please refer to Vault Disaster Recovery documentation in the Client Portal)
...
When Active-Passive mode is used in combination with multiple physical databases (see 7.15.3 Operate Vault Core on Multiple Physical Databases) there is the option to configure delayed replication of the Warm Storage database via a Kafka replication service (e.g. MirrorMaker). This removes the requirement to perform snapshot restoration of the Warm Storage database during failover, which can significantly reduce the recovery time for larger Vault Core deployments.

#### [](#operate_vault_core_on_multiple_physical_databases "Copy link to heading")7.15.3. Operate Vault Core on Multiple Physical Databases

This deployment mode enables Clients to host a single Vault Core instance across multiple physical databases. Specifically, it allows moving the read-optimised logical database (warm storage) and Activities Data (Audit Log, Action Log, Contract Event) logical database to separate physical PostgreSQL databases, while all other logical databases remain in the primary PostgreSQL physical database.

This configuration is intended for Clients with high data volumes, allowing them to bypass the storage and performance limitations of a single physical database.
...
Upon re-evaluation, the Standard Retail performance profile provides all the indicators that Daily and Monthly performance profiles are providing. Therefore, the tests on Daily and Monthly performance profiles will be removed and no longer reported from Vault Core 5.10.0.
...
-   autovacuum\_analyze\_scale\_factor to 0.05
    
-   autovacuum\_analyze\_threshold to 500
    
-   autovacuum\_freeze\_max\_age to 800000000
    
-   autovacuum\_max\_workers to 6
    
-   autovacuum\_multixact\_freeze\_max\_age to 800000000
    
-   autovacuum\_vacuum\_cost\_delay to -1
    
-   autovacuum\_vacuum\_scale\_factor to 0.1
    
-   autovacuum\_vacuum\_threshold to 500
    
-   log\_autovacuum\_min\_duration to 0
    
-   maintenance\_work\_mem to 2097152
    
-   max\_connections to 2000
...
-   max\_replication\_slots to 15
    
-   max\_wal\_senders to 15
    
-   max\_wal\_size to 8192
    
-   password\_encryption to scram-sha-256
    
-   vacuum\_freeze\_min\_age to 5000000
    
-   vacuum\_multixact\_freeze\_min\_age to 5000000
...
-   alloydb.iam\_authentication to on
...
-   1 primary instance, 1 read replica
    
-   Contains all logical databases except Warm Storage, Activity, and Contract Events
...
Activity and Contract Events Database
...
-   Contains the Activity and Contract Events logical databases only
...
-   1 primary cluster + instance, 1 read replica
...
To view the list of deprecated features, review the deprecation notices in the [technical details of this release on the Documentation Hub](/vault-core/5-9/EN/vault_release_information/technical_details_of_this_release#deprecated_features).