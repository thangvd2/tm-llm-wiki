---
source_url: "https://vault-portal.thoughtmachine.net/policy/latest/EN/product_descriptions/vault_core_5_4"
title: "Vault Core Version 5.4 Product Description"
scraped_at: "2026-06-17T15:56:41.984Z"
images: 0
---

# Vault Core Version 5.4 Product Description

[Download PDF](/policy/latest/EN/resources/vault_core_5_4.pdf)

## [](#overview "Copy link to heading")1\. Overview

This product description describes the following for Vault Core Version 5.4
...
Details on the specific components of the Vault Core product and information about the APIs are available on the Thought Machine Documentation Hub website. This product description sets out the Vault Core "Standard" offering. However, this product description may also include optional additional features and functionality (which will be expressly called out using the signpost: "optional add-on") which do not form part of the "Standard" offering and are only included where that add-on has been contractually agreed to be included in the MSA.
...
-   For Postings, a time series of Postings using either the insert or value time
    
-   For Balances, live Account Balances as well as their historic data
...
-   For Denominations, the permitted denominations of an Account \* For Supervisee details, the alias of an Account within a Supervisor Contract
...
#### [](#generate_side_effects_of_financial_movements "Copy link to heading")3.2.2. Generate side effects of financial movements
...
##### [](#generate_notifications "Copy link to heading")3.2.2.1. Generate notifications
...
##### [](#instruct_subsequent_financial_movements "Copy link to heading")3.2.2.2. Instruct subsequent financial movements

Clients can instruct a set of further financial movements on the Account, for example instructing a rebalance on different balances within the Account. For example, once a Posting is accepted then accruing a rewards points balance depending on the amount and type of the Posting.

##### [](#make_a_change_to_a_predefined_scheduled_event "Copy link to heading")3.2.2.3. Make a change to a predefined scheduled event
...
-   Parameter Value Hierarchy Parameters - an optional add-on enabling custom grouping of Accounts or products
...
#### [](#configure_parameters_for_a_group_of_accounts_using_the_parameter_value_hierarchy_optional_add_on "Copy link to heading")3.3.4. Configure Parameters for a group of accounts using the Parameter Value Hierarchy (optional add-on)
...
Scheduled operations that have “Errored” during execution can be manually retried in bulk via an action button in Vault Jobs.

### [](#manage_the_lifecycle_of_products "Copy link to heading")3.6. Manage the lifecycle of products
...
#### [](#launch_a_financial_product "Copy link to heading")3.6.1. Launch a financial product
...
#### [](#update_financial_products "Copy link to heading")3.6.2. Update financial products
...
### [](#simulate_the_behaviour_of_financial_products "Copy link to heading")3.7. Simulate the behaviour of financial products
...
#### [](#simulate_the_behaviour_of_an_account_before_it_is_opened "Copy link to heading")3.7.1. Simulate the behaviour of an Account before it is opened
...
#### [](#simulate_the_behaviour_of_an_existing_customer_account "Copy link to heading")3.7.2. Simulate the behaviour of an existing Customer Account
...
### [](#test_the_correctness_of_the_financial_products "Copy link to heading")3.8. Test the correctness of the financial products

Clients can test product behaviour in a simulated manner without making changes to a Vault Core instance.

#### [](#carry_out_unit_tests_for_financial_products "Copy link to heading")3.8.1. Carry out unit tests for financial products
...
#### [](#carry_out_end_to_end_tests_on_financial_products "Copy link to heading")3.8.2. Carry out end to end tests on financial products

In order to carry out end to end (E2E) tests on logic configured within a Smart Contract, we advise Clients to use Contract Simulation via our Core API.

Clients can provide start and end times, and running the simulation will internally replay all events that would have happened in the real Vault Core. All Smart Contract Hooks (including scheduled code) will run as they would in a real Vault Core instance.

### [](#configure_a_financial_product_to_calculate_and_surface_data_based_on_the_state_of_an_account "Copy link to heading")3.9. Configure a financial product to calculate and surface data based on the state of an Account
...
Vault Core can construct a time series with the value time order which shows the order that Postings should have been applied to the Account(s) but not necessarily when they were; for example, Vault Core can receive a backdated Posting after processing a later Posting. This time series is mutable; the value balance at any point in time can change if Vault Core receives new backdated Postings.

NOTE - It is not possible to put a future date on the Value Timestamp.
...
The Booking Timestamp on a Posting represents the time when the funds movement was booked (accounted for) according to Vault Core’s configured booking periods. In most cases Vault Core sets this value based on the configured booking periods. It is, however, possible for Clients to override Vault Core’s booking periods by explicitly setting this value on the Posting. This enables a Client to decide which accounting period it uses to report the Posting to.

NOTE - It is not possible to put a future date on the Booking Timestamp.
...
Clients can retrieve a time series of Balances of an Account via the Core API by specifying the Account ID along with the desired time range for the Balances. This enables them to retrieve a historical balance.
...
-   specifying a range of attributes associated with the customer. For example, by customer status, email, phone number, and other attributes that you have recorded with the customer details
...
Create either an Accounts for a customer or an Internal Account for your institution using the Core API. When creating the Account, Clients specify values for the Account Parameters defined in the underlying Smart Contract of the product the Account belongs to.
...
Clients can capture the status of an Account to identify the stage it is at in its life cycle, such as “Pending”, “Open” or “Closed”. Clients can also change an Account’s status if and when you need to.
...
#### [](#create_and_configure_multiple_processing_groups_optional_add_on "Copy link to heading")5.8.2. Create and configure multiple Processing Groups (optional add-on)
...
#### [](#manage_bau_operational_activity_via_a_user_interface "Copy link to heading")5.9.1. Manage BAU operational activity via a user interface
...
#### [](#view_a_dashboard_to_track_the_progress_of_scheduled_operations "Copy link to heading")5.9.2. View a dashboard to track the progress of scheduled operations
...
#### [](#retry_errored_vault_jobs_operations "Copy link to heading")5.9.3. Retry Errored Vault Jobs Operations
...
#### [](#view_a_dashboard_to_see_vault_operations_that_have_not_succeeded "Copy link to heading")5.9.4. View a dashboard to see Vault operations that have not succeeded
...
Thought Machines provides the DLQ Inspector as a means for Clients to remediate processes that may have failed for a temporary outage or other issue that is now resolved. There is little to no operational value in storing information about previous errors beyond a reasonable operational window in which retry is considered a valid remediation strategy. Clients can configure the retention period and the schedule for the purging operation in the values.yaml file.

### [](#notify_downstream_systems_about_events_that_have_occurred_in_vault_core "Copy link to heading")5.10. Notify downstream systems about events that have occurred in Vault Core
...
#### [](#select_the_format_for_event_notifications "Copy link to heading")5.10.1. Select the format for event notifications
...
#### [](#aggregate_vault_jobs_notifications_into_meaningful_events_of_interest "Copy link to heading")5.10.2. Aggregate Vault Jobs notifications into meaningful events of interest

Vault Jobs tracks granular jobs across the following dimensions: \* Processing Group ID \* Smart Contract Version ID \* Event type \* Scheduled time
...
### [](#reconcile_that_event_notifications_from_vault_core_have_been_received "Copy link to heading")5.11. Reconcile that event notifications from Vault Core have been received
...
#### [](#detect_missed_events "Copy link to heading")5.11.1. Detect missed events
...
#### [](#replay_missed_events "Copy link to heading")5.11.2. Replay missed events

Clients can replay any missed events by calling an endpoint within the Core API.

The exception to this is Smart Contract notifications which cannot be replayed.

### [](#load_historical_data_from_a_previous_core_banking_system "Copy link to heading")5.12. Load historical data from a previous core banking system
...
The Postings Migration API does not support historical postings directed to custom assets and addresses.

### [](#manage_audit_logs "Copy link to heading")5.13. Manage Audit Logs
...
### [](#manage_action_logs "Copy link to heading")5.14. Manage Action Logs
...
### [](#manage_the_configuration_of_vault_core_resources_in_bulk "Copy link to heading")5.15. Manage the Configuration of Vault Core resources in bulk
...
#### [](#upload_system_configuration_using_a_command_line_utility "Copy link to heading")5.15.1. Upload system configuration using a command line utility
...
### [](#manage_access_to_system_resources "Copy link to heading")5.16. Manage access to system resources

#### [](#define_and_apply_permissions "Copy link to heading")5.16.1. Define and apply Permissions
...
#### [](#define_and_apply_operational_roles "Copy link to heading")5.16.2. Define and apply operational Roles
...
#### [](#define_and_apply_policies "Copy link to heading")5.16.3. Define and apply Policies
...
#### [](#update_permissions_operational_roles_and_policies "Copy link to heading")5.16.4. Update Permissions, operational Roles and Policies
...
#### [](#retrieve_permissions_operational_roles_and_policies "Copy link to heading")5.16.5. Retrieve Permissions, operational Roles and Policies:
...
#### [](#authenticate_api_requests "Copy link to heading")5.16.6. Authenticate API requests
...
### [](#view_usage_metrics_across_a_vault_core_instance "Copy link to heading")5.17. View usage metrics across a Vault Core instance

#### [](#view_the_number_of_live_customer_accounts "Copy link to heading")5.17.1. View the number of live customer accounts
...
#### [](#view_the_number_of_live_customers "Copy link to heading")5.17.2. View the number of live customers
...
### [](#deletion_of_redundant_operational_data "Copy link to heading")5.18. Deletion of redundant operational data

#### [](#deletion_of_redundant_operational_data_2 "Copy link to heading")5.18.1. Deletion of redundant operational data

An optional Data Deleter component is available to delete operationally redundant data. If installed, the job that is created will remove any unused operational Vault Core data, as well as Vault Jobs resources, on a daily basis 90 days after their creation.
...
We recommend using the Istio package that comes with the TMComponent Operator. Alternatively, the Client can run their own Istio Service Mesh provided that it does not restrict or alter the standard Istio APIs.
...
-   Collects metrics and stores metrics for a limited time (Client configurable), with the option for Clients to configure the RemoteWrite feature to write metrics to a compatible system that they manage
...
-   Inflight messages on inbound public Kafka topics may be dropped before they are able to be consumed or processed by Vault Core. These topics must be restored manually by the Client (Please refer to Vault Disaster Recovery documentation in the Docs Hub)
...
#### [](#single_region_active_passive "Copy link to heading")7.15.1. Single-region active-passive
...
common.deployment\_size: extra-small
...
-   Number of account IDs per batch: 10 (ie. effective QPS = Max RPS \* 10)
...
To view the list of deprecated features, review the deprecation notices in the [technical details of this release on the Documentation Hub](/vault-core/5-4/EN/environment_and_installation/vault_release_information#deprecated_features).