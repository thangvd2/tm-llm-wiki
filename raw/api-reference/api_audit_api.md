...
title: "Audit API"
scraped_at: "2026-05-05T20:04:19.048Z"
images: 0
---
...
Due to the fact that requests are audited on a best-effort basis, the Audit API does not satisfy regulatory compliance and should not be used for such purposes.
...
-   *Action Logs*: Record the effects of a request on a Vault Core object (if there are any).
    

A request may therefore have both an Audit Log and Action Log associated with it. Both types of log are immutable, and are created when an event occurs in the Vault Core API; they are then published to Kafka through the [Audit Streaming API](/vault-core/5-8/EN/api/audit_api#audit_streaming_api). Audit and Action logs can be retrieved through the [Audit API](/vault-core/5-8/EN/api/audit_api).

chat\_bubble

For an overview of the general features of the API, see the [API overview](/vault-core/5-8/EN/api/overview/).
