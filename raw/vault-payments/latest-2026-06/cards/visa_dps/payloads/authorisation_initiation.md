---
source_url: "https://vault-portal.thoughtmachine.net/vault-payments/latest/EN/cards/visa_dps/payloads/authorisation_initiation"
title: "Authorisation initiation"
scraped_at: "2026-06-17T15:50:23.477Z"
images: 0
---

# Authorisation initiation

Authorisation initiation transactions place a temporary hold on a cardholder’s funds, signalling a commitment without immediately moving the money.

An authorisation initiation is a request or advice to reserve a specific amount against the cardholder’s available balance or credit limit. It does not definitively impact the ledger for settlement. These transactions are commonly used for credit card purchases, hotel reservations, or car rentals.

## [](#authorisation_initiation_request_0100 "Copy link to heading")Authorisation initiation request (0100)

A request is an active inquiry seeking real-time permission to reserve funds. The card network asks the receiver to verify the account and approve the temporary hold.

-   **State:** The transaction is pending. Funds are held but not yet transferred, requiring a subsequent clearing or financial message (such as a 0220) to complete the movement of funds.
    
-   **Decision:** The response contains a code indicating whether the authorisation hold is approved or declined (for example, due to insufficient available credit, an invalid card, or fraud controls).
    
-   **Common use case:** A customer swiping a credit card at a retail point-of-sale (POS) or checking into a hotel where an estimated amount for incidentals is reserved.
    

## [](#authorisation_initiation_advice_0120 "Copy link to heading")Authorisation initiation advice (0120)

An advice is an informational notification. The card network informs the receiver that an authorisation hold has already been granted and that the receiver must update their records to reflect the reserved funds. The sender does not ask for permission.

-   **State:** The authorisation has already been completed by an intermediary or alternate method.
    
-   **Decision:** Because the action has already occurred, the issuer cannot decline the advice. The response acts purely as an acknowledgement of receipt.
    
-   **Common use cases:**
    
    -   Offline authorisation: A terminal or EMV chip approves a transaction locally based on risk parameters. Once back online, the terminal sends an advice to inform the bank of the authorisation.
        
    -   Stand-in processing (STIP): A payment network approves an authorisation request on behalf of an unresponsive issuer and sends an advice later to ensure the issuer places the hold on the cardholder’s account.
        
    

## [](#incremental_authorisation "Copy link to heading")Incremental Authorisation

Incremental authorisation refers to the process of requesting an increase to the amount of an existing, approved preauthorisation. Instead of initiating a new, standalone authorisation that would place a separate hold on the cardholder’s account, the merchant links the new request to the original transaction to increase the total reserved funds.

This process is commonly used in industries where the final cost of goods or services can easily exceed the initial estimate, such as hotel stays where a guest extends their stay or incurs room service charges.

### [](#incremental_authorisation_request_0100 "Copy link to heading")Incremental authorisation request (0100)

An active inquiry seeking real-time permission to increase the financial hold of an existing preauthorisation. The card network asks the receiver for permission to reserve the additional funds.

-   **State:** The incremental request is pending. It cannot be completed until the receiver responds.
    
-   **Decision:** The response contains a code indicating whether the request to increase the hold is approved or declined.
    
-   **Common use cases:** A ride-hailing service preauthorises the estimated fare when the trip is booked. But the traffic is busy and significantly increases the time-based fare, the app requests an increment to secure the additional funds.
    

### [](#incremental_authorisation_advice_0120 "Copy link to heading")Incremental authorisation advice (0120)

A notification informing the receiver that an incremental hold has already been placed on the account. The sender does not ask for permission, but advises the receiver to increase the hold amount on their ledgers.

-   **State:** The incremental hold has already been applied.
    
-   **Decision:** Because the action has already occurred, the issuer cannot decline the advice. The response acts purely as an acknowledgement of receipt.
    
-   **Common use cases:** A merchant terminal loses connectivity but is configured to accept minor incidental charges locally, sending an advice to increase the hold once back online.
...
Mapped to delayed charges, no-show, re-submission, reauthorisation flags. In Visa DPS messages, the incremental authorisation flag is set to 1.
...
Cardholder.ContactInformation.OtherEmail
...
### [](#example_authorisation_initiation_request_payload "Copy link to heading")Example authorisation initiation request payload