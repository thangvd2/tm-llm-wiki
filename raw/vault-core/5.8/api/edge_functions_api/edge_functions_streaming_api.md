---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-8/EN/api/edge_functions_api/edge_functions_streaming_api"
title: "Edge Functions Streaming API"
scraped_at: "2026-06-17T15:39:07.642Z"
images: 0
---
...
This mirrors the functionality in [Core Streaming API](/vault-core/5-8/EN/api/core_api#core_streaming_api). Most of what applies to the Core Streaming API also applies to Edge Functions resources with a few specific differences.
...
For more information, see [Event reconcilliation](#event_reconciliation).
...
If you have detected missed events by using the `change_id`, you can replay these events by calling the Edge Functions `Replay Journal Events` endpoint ([POST {edge-functions-api-url}/v1/journal-events:replay](/vault-core/5-8/EN/api/edge_functions_api#_journal_events_ReplayJournalEventsResponse_ReplayJournalEvents)). Calling this endpoint cuases the system to replay an exact copy of the original events for downstream consumption, with an added batch ID header with key `X-JournalEventsBatch-ID`.

chat\_bubble

You can replay events for the `resource_type`s listed within `List Journal Events` ([GET {edge-functions-api-url}/v1/journal-events](/vault-core/5-8/EN/api/edge_functions_api#_journal_events_ListJournalEventsResponse_ListJournalEvents)) results.
...
There are two errors that can occur when reconciling events:
...
The errors described below describe `now()` as the time at which the Edge Functions Journal Events API starts processing a request made to either the `Get Journal Events Checksum` endpoint ([GET {edge-functions-api-url}/v1/journal-events:checksum](/vault-core/5-8/EN/api/edge_functions_api#_journal_events_GetJournalEventsChecksumResponse_GetJournalEventsChecksum)) or to the `List Journal Events` endpoint ([GET {edge-functions-api-url}/v1/journal-events](/vault-core/5-8/EN/api/edge_functions_api#_journal_events_ListJournalEventsResponse_ListJournalEvents)).
...
If you are using [Journal Events](/vault-core/5-8/EN/api/core_api#journal_events) to detect and replay missed events over that period, you should therefore only replay ones that have `journal_events[].published` set to `true`.
...
An `EdgeFunctionEvent` is published when an [EdgeFunction](/vault-core/5-8/EN/api/edge_functions_api#edgefunction) resource is created or updated.
...
An `EdgeFunctionExecutionEvent` is published when an Edge Function is executed. This will create and possibly update an [EdgeFunctionExecution](/vault-core/5-8/EN/api/edge_functions_api#edgefunctionexecution) resource that is sent out with events.
...
An `EdgeFunctionVersionEvent` is published when an [EdgeFunctionVersion](/vault-core/5-8/EN/api/edge_functions_api#edgefunctionversion) resource is created.
