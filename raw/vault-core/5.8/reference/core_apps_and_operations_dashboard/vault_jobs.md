---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-8/EN/reference/core_apps_and_operations_dashboard/vault_jobs"
title: "Vault Jobs"
scraped_at: "2026-06-17T15:42:04.384Z"
images: 8
---

# Vault Jobs

Welcome to the Vault Jobs application documentation.
...
Vault Jobs is a Web Application that displays the progress and health of long-running account processes within Vault, such as End of Day. For example:

![vault\_jobs\_page\_mock\_complete\_with\_errors.png](_assets/vault_jobs_list_page.CZrz03Wa_ZpzPiJ_vaultcor.webp)
...
-   Logged-in users can view the progress of Schedule Jobs, grouped by various attributes of the specific Vault Job - see [How Vault Jobs are created](/vault-core/5-8/EN/reference/core_apps_and_operations_dashboard/vault_jobs#how_vault_jobs_are_created)
    
-   Remediating errored Schedule Jobs (Vault Job operations) by retrying them in bulk once underlying issue with a Scheduled Job is fixed. For example, syntax error in the Smart Contract is fixed and impacted Accounts are converted to the new Smart Contract version. You can monitor remediation via the **Remediation Processor** panels on the [Vault Jobs Overview dashboard](/vault-core/5-8/EN/environment_and_installation/infrastructure_docs/observability_and_monitoring/introduction_to_grafana_and_key_dashboards#vault_jobs_overview_dashboard)
...
The following diagram shows the Core API Parameters integration, where a point in time change to a globally owned and/or [Parameter Value Hierarchy](/vault-core/5-8/EN/reference/parameters/building_and_managing_the_parameter_value_hierarchy) node owned value triggers the post\_parameter\_change\_hook (for Accounts that are opted in to the hook), which in turn generates a Vault Job operation:
...
-   [Scheduler overview](/vault-core/5-8/EN/reference/scheduler#glossary)
    
-   [Smart Contracts overview](/vault-core/5-8/EN/reference/contracts/)
    
-   [Smart Contracts Event Types](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#smartcontracteventtype)
    
-   [Core API ParameterValue resource](/vault-core/5-8/EN/api/core_api#parametervalue)
    
-   [Parameters](/vault-core/5-8/EN/reference/parameters)
    

## [](#how_vault_jobs_are_created "Copy link to heading")How Vault Jobs are created

*Vault Jobs* and *Vault Job operations* are created automatically when either Scheduler Jobs, or Parameter value changes (above account level) occur within Vault.
...
-   [Processing Groups](/vault-core/5-8/EN/reference/processing_groups)
    
-   Smart Contract version (see the [Smart Contracts overview](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/overview)) or Supervisor Contract version (see the [Supervisor Contracts overview](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_overview/))
    
-   Event Type - as defined within the Smart Contract (see [Smart Contract Event Type](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#smartcontracteventtype))
    
-   Scheduled timestamp. This is the time when the Schedules are set to execute (see [Scheduler Overview](/vault-core/5-8/EN/reference/scheduler#schedules))
...
A *Parameter change* Job is created any time that a Parameter value change comes into effect, where the owner of the value(s) are global and/or one or more nodes in [the Parameter Value Hierarchy](/vault-core/5-8/EN/reference/parameters/building_and_managing_the_parameter_value_hierarchy).
...
If you are using the Adjustments Extension, see [Monitoring Adjustment progress](/vault-core/5-8/EN/reference/adjustments#monitoring_adjustment_progress) for details on how the Vault Jobs App integrates with Adjustments.
...
If the optional [Data Deleter Job](/vault-core/5-8/EN/environment_and_installation/infrastructure_docs/tmcomponent_operator_user_guide/advanced_installation_options#deploying_data_deleter_job) is enabled, the Vault Jobs app only displays 90 days of Vault Jobs history.
...
Click on any of the errored operations to expand the row and view [additional information](/vault-core/5-8/EN/reference/core_apps_and_operations_dashboard/vault_jobs#vault_jobs_field_descriptions) about the affected Account or Plan:
...
For more information about the fields in the panels, see [Vault Jobs Field Descriptions](/vault-core/5-8/EN/reference/core_apps_and_operations_dashboard/vault_jobs#vault_jobs_field_descriptions).
...
The ID of [the Parameter Value Hierarchy](/vault-core/5-8/EN/reference/parameters/building_and_managing_the_parameter_value_hierarchy) node that owns the Parameter value.
...
Error details (such as type and stack\_trace) returned by the failed Vault Job’s operation.
...
For a list of possible statuses and other event information, see [Vault Job Events](/vault-core/5-8/EN/api/core_api#vault_jobs_events).
