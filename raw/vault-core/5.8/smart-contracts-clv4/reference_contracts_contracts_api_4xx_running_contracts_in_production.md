---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/running_contracts_in_production"
title: "Running Contracts in production"
scraped_at: "2026-06-16T16:37:34.952Z"
images: 4
---

# Running Contracts in production

In order to help monitor and debug Smart and Supervisor Contracts that are running in production, Vault Core uses [Contract Events](/vault-core/5-8/EN/api/core_api#contract_events), which contain sequences of [Contract Executions](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/running_contracts_in_production#contract_events_and_the_contract_executions_api).
...
-   scheduled\_event\_hook [for Smart Contracts](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/smart_contracts_api_reference4xx/hooks#scheduled_event_hook) and [for Supervisor Contracts](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_contracts_api_reference4xx/hooks#scheduled_event_hook)
    
-   post\_posting\_hook [for Smart Contracts](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/smart_contracts_api_reference4xx/hooks#post_posting_hook) and [for Supervisor Contracts](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_contracts_api_reference4xx/hooks#post_posting_hook)
    
-   [post\_parameter\_change\_hook](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/smart_contracts_api_reference4xx/hooks#post_parameter_change_hook), except if the parameter value creation was triggered via [v1/account-updates](/vault-core/5-8/EN/api/core_api#accountupdate).
    

The [Accounts V1 APIs](/vault-core/5-8/EN/api/core_api#accounts_version_1) are now superseded by [Accounts V2 APIs](/vault-core/5-8/EN/api/core_api#accounts_version_2). In line with this, creating ParameterValues via [v1/account-updates](/vault-core/5-8/EN/api/core_api#accountupdate) is also now superseded by [v1/parameter-values](/vault-core/5-8/EN/api/core_api#parametervalue). Executions of [post\_parameter\_change\_hook](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/smart_contracts_api_reference4xx/hooks#post_parameter_change_hook) triggered via [v1/parameter-values](/vault-core/5-8/EN/api/core_api#parametervalue) can cause the creation of ContractExecutions.
...
-   A Supervisor Contract invokes the Smart Contract hooks of supervisee accounts. See section [Contract Execution supervision model](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/running_contracts_in_production#contract_execution_supervision_model).
    
-   Vault Core retries a Contract Event after an error, either automatically, or due to a manually-triggered retry. See section [Creation of Contract Execution data on retryable errors](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/running_contracts_in_production#creation_of_contract_execution_data_on_retryable_errors).
...
![Example Contract Event in progress](_assets/contracteventattempts.CprgwAZq_QgFlG_vaultcor.webp)
...
![Configuring a Supervisor Contract to invoke hooks](_assets/contractexecsupervision.DZTaWKGw_Z2lt1AK_vaultcor.webp)

1.  The Supervisor Contract declares that hooks of supervisee accounts' Smart Contract be invoked.
    
2.  One way to configure this is to use the [`supervisee_hook_directives`](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_contracts_api_reference4xx/hook_requirements#supervisee_hook_directives) keyword in the [`@requires`](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_types_4xx/decorators#requires) decorator of a Supervisor Contract `scheduled_event_hook`.
...
There is a Contract Execution resource created for each invoked supervisee Smart Contract hook, and a Contract Execution for the Supervisor Contract hook. See [Contract Execution](/vault-core/5-8/EN/api/core_api#contractexecution).
...
-   A Core API endpoint was triggered to manually retry the contract event. For example, this can occur when calling [BatchRepublishJob](/vault-core/5-8/EN/api/core_api#job) or [RepublishPostPostingFailure](/vault-core/5-8/EN/api/core_api#postpostingfailure).
...
See information on [deploying the data deleter](/vault-core/5-8/EN/environment_and_installation/infrastructure_docs/tmcomponent_operator_user_guide/advanced_installation_options#deploying_data_deleter_job).
...
![Scheduled event has error](_assets/contractsevent_example1.CPTB2vVd_2pzo9b_vaultcor.webp)
...
![Incorrect interest accrual](_assets/contractsevent_example2.CycFaNTN_Z1H1vhd_vaultcor.webp)