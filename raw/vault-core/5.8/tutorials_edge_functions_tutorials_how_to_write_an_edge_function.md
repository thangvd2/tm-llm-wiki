---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function"
title: "How to write an Edge Function"
scraped_at: "2026-06-17T05:35:27.130Z"
images: 1
---
...
-   Key aspects of the Edge Functions API for creating, updating, and managing the current version and status of an Edge Function
    

Before you continue, check the [Quick start guide](/vault-core/5-8/EN/reference/edge_functions/overview_and_getting_started/quick_start_guide) for an overview and prerequisites to make sure that you have everything you need to begin writing the Edge Function code.
...
![Edge Function and Edge Function Version resources](_assets/edge_functions_and_versions_relationship_vaultcor.svg)

When you create your first Edge Function, you also create its first Edge Function Version.

An `EdgeFunction` resource can have multiple `EdgeFunctionVersion` subresources, and you must assign one as the `current_edge_function_version`. When you make a request to execute an Edge Function, it executes the specified current version unless you specify otherwise in the request.
...
Thought Machine has used the [Pydantic library in the Edge Functions SDK to implement functionality](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function#pydantic_objects_in_edge_functions). This includes using Pydantic’s `BaseModel` class to perform validation and serialisation, and the ability to derive JSON schema from models as with the request and response types.
...
-   [`ExecutionContext`](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#executioncontext) – injected into the handler that provides `idempotency_key` and `execution_id`
    
-   [`Model`](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#model) – input/output classes render from this
...
Edge Functions only allow you to import a subset of Python objects due to security reasons. For more information, see [Allowed Python imports in Edge Functions](/vault-core/5-8/EN/reference/edge_functions/allowed_imports).
...
-   `entry_point` – marks the `entry_point` function. See: [entry\_point](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function#entry_point_decorator) decorator
...
-   `ExecutionContext` – surfaces common execution parameters to the execution. See: [ExecutionContext](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function#executioncontext)
    
-   `Logger` - logger creator for logging information. See: [Logging](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function#logging)
    

It also imports the `Model` to subclass the request and response classes from, and the associated `model_field` function for defining constraints and other metadata on the fields of a `Model`. See [base class for request and response types](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function#base_class_for_request_and_response_types_edge_api_model) for further information about the use of `Model` and `model_field`.
...
When using VC API to make calls to Vault Core, if a request receives a response other than HTTP 200, the library automatically converts it to an error response. See the [API Reference](/vault-core/5-8/EN/api/edge_functions_api) for a list of error codes and [How to troubleshoot Edge Functions](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/troubleshooting).
...
You can find more information about logging and checking logs for errors later in this guidance in the [Logging](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function#logging) section.
...
The fields within [`Error`](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#error) are:
...
Populates the error code, either one of the [preconfigured enum values](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/troubleshooting#debugging_execution_failures) or a custom value if you have subclassed [`ErrorCodeEnum`](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#errorcodeenum).
...
Exceptions raised by the function which are not [`Error`](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#error) are translated into an `Error` instance and then handled in the same way as described for the `Error` object. The code is set to `FUNCTION_RAISED_EXCEPTION` and details contain the type, message, and stack trace of the exception.

##### [](#example_error_using_pydantic_serialisation "Copy link to heading")Example error using Pydantic serialisation

Errors serialise to JSON. For example, a custom error subclassed with [`ErrorCodeEnum`](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#errorcodeenum) could serialise to the following helpful output:

### [](#implementing_custom_errors "Copy link to heading")Implementing custom errors

The Edge Functions SDK provides an [`Error`](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#error) type which you can raise to signal that an error has occurred. You can construct an enum class that bases [`edge_api.ErrorCodeEnum`](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#errorcodeenum) to define a set of custom error codes that could occur while using Edge Functions.
...
Use to populate the error code. For acceptable values, see the [preconfigured enum values](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/troubleshooting#debugging_execution_failures). You can also define your own error codes by subclassing `ErrorCodeEnum`.

 |

The `Error` class has the following attributes – for more information, see the [SDK reference](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#error).
...
Populates the error code; by default, this is one of the [preconfigured enum values](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/troubleshooting#debugging_execution_failures). You can also define your own error codes by subclassing `ErrorCodeEnum`, as shown [here](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function#example_with_error)
...
To do this, you could attempt to execute an Edge Function in such a way that you would expect to return a given error.
...
-   Any of the built-in ErrorCode values - see the [Edge Functions SDK reference](/vault-core/5-8/EN/reference/edge_functions/sdk_reference), [How to troubleshoot Edge Functions](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/troubleshooting), and [Monitoring and Disaster Recovery](/vault-core/5-8/EN/reference/edge_functions/overview_and_getting_started/observability_and_disaster_recovery) for Edge Functions
...
-   [How to troubleshoot Edge Functions](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/troubleshooting)
    
-   [Monitoring and Disaster Recovery](/vault-core/5-8/EN/reference/edge_functions/overview_and_getting_started/observability_and_disaster_recovery) for Edge Functions
    
-   [Using the Observability Stack](/vault-core/5-8/EN/environment_and_installation/infrastructure_docs/observability_and_monitoring/using_the_observability_stack) for Vault Core (under **Metrics common labels**)
...
To learn more about the underlying Pydantic models used by Edge Functions, see the [Edge Functions SDK reference](/vault-core/5-8/EN/reference/edge_functions/sdk_reference) and the [Pydantic developer documentation](https://docs.pydantic.dev/2.6/).
...
The following snippet builds on the [previous example](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function#overview_of_edge_function_source_code) to include the following types under the `edge_api` imports:

-   `[Session](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#session)`
    
-   `[VaultAPI](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/edge_api_library_reference#vaultapi)`
    

These instantiate `[CoreAPIClient](/vault-core/5-8/EN/reference/edge_functions/sdk_reference/vc_api_library_reference)` from the `vc_api`, which calls the [Vault Core API](/vault-core/5-8/EN/api/core_api).
...
You can find detailed guidance about using the CLU or the API to create an Edge Function in [How to upload an Edge Function](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_upload_an_edge_function).
...
To update the source code of an Edge Function resource, you must create a new Edge Function Version.

There are two ways that you can achieve this via the API, using the following endpoints:

-   [`CreateEdgeFunctionVersion`](/vault-core/5-8/EN/api/edge_functions_api#_edge_functions_api_v1_EdgeFunctionVersion_CreateEdgeFunctionVersion)
    
-   [`UpdateEdgeFunction`](/vault-core/5-8/EN/api/edge_functions_api#_edge_functions_api_v1_EdgeFunctionVersion_UpdateEdgeFunctionVersion)
    

If you want to update an existing Edge Function, see [How to update an Edge Function](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_update_an_edge_function).
...
For more information, see [List Edge Functions](/vault-core/5-8/EN/api/edge_functions_api#_edge_functions_api_v1_ListEdgeFunctionsResponse_ListEdgeFunctions).
...
Alternatively, you can return an individual Edge Function by making a `GET` request to `/v1/edge-functions/{id}`. For more information, see [Get an Edge Function](/vault-core/5-8/EN/api/edge_functions_api#_edge_functions_api_v1_EdgeFunction_GetEdgeFunction).
...
-   [How to upload an Edge Function](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_upload_an_edge_function)
    
-   [How to write a unit test for an Edge Function](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_write_a_unit_test_for_an_edge_function)
    
-   [How to test an Edge Function](/vault-core/5-8/EN/tutorials/edge_functions_tutorials/how_to_test_an_edge_function)