---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-8/EN/reference/calendar"
title: "Calendars"
scraped_at: "2026-05-05T20:05:54.165Z"
images: 0
---
...
In Vault Core 5, support for pausing and resuming End-of-Day processing is available using [Processing Groups](/vault-core/5-8/EN/reference/processing_groups).
...
![A diagram showing an overview of a Vault Core Calendar](/vault-core/5-8/EN/_astro/calendar_overview.0Mu_2VMX_Lcmrp.webp)
...
-   A `start_timestamp` of 1 March 2024 00:00:00
    
-   An `end_timestamp` of 2 March 2024 00:00:00
    

Calendar Events are streamed out upon creations and updates, to the Core Streaming API [CalendarEvents](/vault-core/5-8/EN/api/core_api#calendar_events) topic.
...
In order to use the Calendar Period functionality, a Calendar must have a [Calendar Period Descriptor](/vault-core/5-8/EN/reference/calendar#calendar_period_descriptor) associated with it.

Components integrating with Vault Core can get a unique identifier for the current cycle (`period`) by making a request through the Core API for the [Calendar Period](/vault-core/5-8/EN/api/core_api#calendarperiod). You can also request the corresponding cycle for a provided point in time in the past or the future.

In addition to requesting Period details on demand, [Period Creation Events](/vault-core/5-8/EN/api/core_api#calendarperiodevent) are published when a Calendar Period begins.

You can change the current Calendar Period by applying a [Hold or Roll](/vault-core/5-8/EN/reference/calendar#manipulating_holding_and_rolling_periods) action. A Hold action extends the end timestamp of a period, whereas a Roll action shortens the end timestamp of a period. Action timestamps for both Hold and Roll must be in the future.
...
The periods, as they advance in time, are streamed out as events on the [CalendarPeriodEvent](/vault-core/5-8/EN/api/core_api#calendarperiodevent) Core streaming API topic on an *at least once* basis. Services integrating with Vault Core may listen to these period events based on the period descriptor associated with the Calendar they have created.
...
-   Rolling the current period will cause the next period to start earlier (as it must occur immediately after the end of the prior period) and have a longer timespan (unless it is also subsequently rolled early)
...
![A timeline illustrating the effects of HOLD and ROLL actions](/vault-core/5-8/EN/_astro/calendar_hold_and_roll.BputUYxM_Z1L1w0k.webp)