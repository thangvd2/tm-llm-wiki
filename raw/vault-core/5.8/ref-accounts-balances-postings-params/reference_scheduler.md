---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-8/EN/reference/scheduler"
title: "Scheduler"
scraped_at: "2026-05-05T20:05:57.323Z"
images: 0
---
...
The Scheduler is a time-based Kafka producer. It publishes triggers in the form of Kafka messages to the designated topics. These messages are called Schedule jobs, and are published at regular intervals or single points in time as required by other parts of Vault Core to fulfil the business logic.

The following diagram shows the components involved:

![Scheduler\_\_Components\_involved.png](/vault-core/5-8/EN/_astro/uuid-aa66eb79-ffbd-d1a4-d7cd-91ac5370a1be-en.bDDqalgG_Z1TMsY4.webp)

## [](#concepts "Copy link to heading")Concepts
...
You create Schedules using Scheduler clients such as Smart Contracts or Workflows. See the respective [Smart Contracts](/vault-core/5-8/EN/reference/contracts/) and [Workflows](/vault-core/5-8/EN/reference/workflows-tickets/) documentation to learn how to schedule events in these services.
...
You can retrieve Schedules through the Core API. See the Core API’s [Schedule](/vault-core/5-8/EN/api/core_api#schedule) documentation for more information.
...
You can retrieve Schedule jobs through the Core API. See the Core API’s [Job](/vault-core/5-8/EN/api/core_api#job) documentation for more information.

You can also view the jobs created by the Scheduler from the *Vault Jobs* application. For more information, see [What is Vault Jobs?](/vault-core/5-8/EN/reference/core_apps_and_operations_dashboard/vault_jobs#what_is_vault_jobs).

### [](#republishing_schedule_jobs "Copy link to heading")Republishing Schedule Jobs

#### [](#overview_2 "Copy link to heading")Overview

You can republish Schedule Jobs using the [BatchRepublish](/vault-core/5-8/EN/api/core_api#job) endpoint.
...
You may still need to apply any directives associated with the failed Schedule that may have been missed. However, this is limited to the failed event itself, rather than every event between the failure and the present. You can do this using the [Simulate](/vault-core/5-8/EN/api/core_api#contracts) endpoint.
...
To start using operation events, you must create a tag. Tags are string labels that Scheduler clients can link to schedules. You can create tags using the [AccountScheduleTag](/vault-core/5-8/EN/api/core_api#account_schedule_tags) endpoint in the Core API.

Once created, within Smart Contracts, you can add tags to Schedules. The collection of all Schedules within the system with exactly the same tag is called a scheduled operation. See [SmartContractEventType tags](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#smartcontracteventtype).
...
In the Scheduler documentation, a Schedule Group is a resource that defines an ordered set of schedules. In Smart Contracts, a Scheduler group is a Group resource defined by the Scheduler. See [Event types groups and tags](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_examples/generic#event_types_groups_and_tags) for more information.
...
Grouped Schedules run based on their time expression, but are blocked until all prior scheduled Jobs have been successfully executed. For more information, see [Schedule jobs](/vault-core/5-8/EN/reference/scheduler#jobs).
...
Schedule jobs can fail, for example due to Smart Contract programming errors, or if a Posting is made that is rejected by the Posting Processor where the rejection reason is not marked as [non-blocking](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_examples#non_schedule_blocking_posting_rejections) by the Smart Contract. This has an effect on whether future jobs are published:
