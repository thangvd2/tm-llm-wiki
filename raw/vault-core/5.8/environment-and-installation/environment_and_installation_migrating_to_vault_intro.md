---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/intro"
title: "Introduction to the Migration APIs"
scraped_at: "2026-06-16T15:24:02.357Z"
images: 2
---
...
-   **Data Loader API** (used to migrate majority of data to Vault Core except Postings)
    ...
    -   [Guidance](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/migrating_using_the_data_loader_api)
        
    -   [API specifications](/vault-core/5-8/EN/api/data_loader_api/)
        
    
-   **BAU Postings API** (alternative means of migrating Postings)
    
    -   [Guidance](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/migrating_using_the_postings_migration_api)
        
    -   [API specifications](/vault-core/5-8/EN/api/postings_api#synchronous_postings_api)
...
-   [asynchronous (Kafka) Postings API](/vault-core/5-8/EN/api/postings_api#asynchronous_postings_api)
    
-   [synchronous (REST) Postings API](/vault-core/5-8/EN/api/postings_api#synchronous_postings_api)
...
Postings that are submitted via the BAU Postings API, without intervention, go through all [postings validation checks](/vault-core/5-8/EN/reference/postings#postings_checks). As a result some additional effort is required to enable this API to correctly support migration use cases.
...
The Posting Migration API request topic provides the same interface as the BAU Posting API request topic, accepting messages in the format of [CreatePostingInstructionBatchRequest](/vault-core/5-8/EN/api/core_api#posting_instruction_batches).
...
-   [Selecting API message formats](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/intro#selecting_json_versus_proto_api_formats).
    

See the [TMComponent Operator User Guide](/vault-core/5-8/EN/environment_and_installation/infrastructure_docs/tmcomponent_operator_user_guide) for guidance on installing Vault components using the `operator-install` method.
...
Regardless of your hosting method, it is important to consider that during the `Migration` component installation a message format is selected for the [Data Loader API](/vault-core/5-8/EN/api/data_loader_api/) and [Posting Migration API / BAU Postings API](/vault-core/5-8/EN/api/postings_api#posting_migration_api).

This is set within the `values.yaml` file, and has independent values as follows:

-   `postings.api.format` ([Posting Migration API](/vault-core/5-8/EN/api/postings_api#posting_migration_api) & [BAU Postings API](/vault-core/5-8/EN/api/postings_api#using_the_asynchronous_api) request and response messages).
    
-   `common_stream_api.message_format` ([Core API](/vault-core/5-8/EN/api/core_api#core_streaming_api) streamed events. This includes the `PostingInstructionBatchCreatedEvent`, `AccountBalanceEvent` and `BalanceEvent`, as though these are Posting related they are streamed via the Core API).
    
-   `data_loader_api.message_format` ([Data Loader API](/vault-core/5-8/EN/api/data_loader_api/) request and all streamed Data Loader events).
...
-   Using the Vault Core [performance report](/vault-core/5-8/EN/vault_release_information/performance_and_testing#performance_report) as a starting point to size your migration environment appropriately.
    
    -   Make sure to set the [database settings](/vault-core/5-8/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/using_a_relational_database/) as per those listed in the AWS / GCP performance reports. For example, set `max_locks_per_transaction` to `2048`.
...
The Data Loader API now supports the migration of data into the new [ParameterValues](/vault-core/5-8/EN/api/core_api#parametervalue) Resource. For the first time, this enables the migration of historic parameters (in addition to any current or active parameters loaded during Account creation). Any Smart Contract operations that rely on a parameter timeseries now have access to the necessary historic data.

Download the [Vault Core Migrations - Data Dictionary](/vault-core/5-8/EN/resources/migrations-data-dictionary.zip) for detailed descriptions of the ParameterValues fields and behaviour.
...
Download the [Vault Core Migrations - Data Dictionary](/vault-core/5-8/EN/resources/migrations-data-dictionary.zip) for detailed descriptions of the Accounts v2 fields.
...
##### [](#posting_migration_api_supports_migration_of_a_historic_insertion_balance_timeseries "Copy link to heading")[Posting Migration API](/vault-core/5-8/EN/api/postings_api#posting_migration_api) supports migration of a historic insertion balance timeseries
...
For more information, see [Posting APIs validation](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/migrating_using_the_postings_migration_api#posting_apis_validation).
...
For more information, see [Preventing BAU Postings prior to migration completing](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/migrating_using_the_postings_migration_api#preventing_bau_postings_prior_to_migration_completing).
...
For more information, see [Bank-hosted](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/intro#bank_hosted) migration.
...
For more information, see considerations for [Migrating Ledger Balances using the Posting APIs](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/migrating_using_the_postings_migration_api#migrating_ledger_balances_using_the_posting_apis).
...
For more information, see [Posting APIs topics and events](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/migrating_using_the_postings_migration_api#posting_apis_topics_and_events).

##### [](#dedicated_posting_migration_api_response_topic "Copy link to heading")Dedicated Posting Migration API Response Topic

The Posting Migration API now has its own dedicated [Posting Response topic](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/migrating_using_the_postings_migration_api#posting_apis_topics_and_events). Every single request passed to the Posting Migration API Request topic (that does not DLQ) now receives streamed events.
...
For more information, see [Posting APIs idempotency](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/migrating_using_the_postings_migration_api#posting_apis_idempotency).
...
For more information, see [Posting APIs monitoring recommendations](/vault-core/5-8/EN/environment_and_installation/migrating_to_vault/migrating_using_the_postings_migration_api#posting_apis_monitoring_and_error_handling).

#### [](#field_level_changes_2 "Copy link to heading")Field-level changes

Download the [Vault Core Migrations - Data Dictionary](/vault-core/5-8/EN/resources/migrations-data-dictionary.zip) for detailed descriptions of the request message structure for Postings in Vault Core 5.