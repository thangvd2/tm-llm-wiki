---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/reference/accounts/switching_from_v1_to_v2_accounts_api"
title: "Switching from the v1 to v2 Accounts API"
scraped_at: "2026-06-17T05:01:02.085Z"
images: 1
---

# Switching from the v1 to v2 Accounts API

This section explains how to perform the complete switch from the `/v1/accounts` and `/v1/internal-accounts` endpoints to the `/v2/accounts` endpoints and associated Streaming API topics. We deem this switch complete when your ecosystem around Vault Core no longer uses these endpoints and associated Account events topics, and instead uses the `/v2/accounts` endpoints and Account events v2 topic as shown in the below table:

   
| /v1/accounts endpoints | /v2/accounts endpoints | Account events v1 topics | Account events v2 topic |
| --- | --- | --- | --- |
| 
-   [v1/accounts/](/vault-core/5-9/EN/api/core_api#account)
    
-   [/v1/accounts/{account\_id}:updateDetails](/vault-core/5-9/EN/api/core_api#accountdetail)
    
-   [/v1/internal-accounts](/vault-core/5-9/EN/api/core_api#internalaccount)
    
-   [/v1/account-migrations](/vault-core/5-9/EN/api/core_api#accountmigration)
...
-   [/v2/accounts](/vault-core/5-9/EN/api/core_api#account)
    
-   [/v2/account-migrations](/vault-core/5-9/EN/api/core_api#accountmigration_2)
...
![Vault\_Core\_5\_0\_\_diagrams\_-\_v1\_\_\_v2\_Accounts\_switchover.svg](_assets/uuid-09e4ec9f-5e17-7299-e250-7f5abd6d785_vaultcor.svg)

## [](#usable_v1_accounts_endpoints_after_the_switch "Copy link to heading")Usable v1 Accounts endpoints after the switch

When the switch to the `/v2/accounts` endpoint and Account events v2 topic is complete, you can still use the following endpoints; however, they are not compatible with High-volume Accounts, Account Attributes or Core API `expected_parameters`:

-   [/v1/account-updates](/vault-core/5-9/EN/api/core_api#accountupdate) and the `vault.core_api.v1.accounts.account_update.events` topic
    
-   [/v1/account-update-batches](/vault-core/5-9/EN/api/core_api#accountupdatebatch) and the `vault.core_api.v1.accounts.account_update_batch.events` topic
    

lightbulb

If you are using derived parameters, you can use the [DerivedParameterValue](/vault-core/5-9/EN/api/core_api#derivedparametervalue) resource to continue retrieving the calculated values after switching to the v2 Accounts API. Vault Core 5 introduces an enhanced version of this feature - for more information see [Account Attributes](/vault-core/5-9/EN/reference/accounts/account_attributes/).

## [](#recommended_simplification_to_client_deployments "Copy link to heading")Recommended simplification to client deployments

Due to the previous behaviour of `/v1/accounts`, client deployments may have incorporated both the synchronous and asynchronous (streaming) API calls into a single function. However, because `v2/accounts` is now a fully synchronous endpoint, these functions need to be reworked. This affects integrations handling the following BAU journeys:

-   Account opening
    
-   Singular Account conversions (made via v1/accounts)
...
-   Instance Parameter value updates
    ...
    While Instance Parameter Values exist in Vault Core 5, they are deprecated in favour of Account-owned [Parameter](/vault-core/5-9/EN/reference/parameters) values.
...
-   All Accounts are backed by [Smart Contract Language version 4](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/) or higher
...
-   You will not be migrating Data into Vault Core at the same time as performing the switch to `/v2/accounts`. See [Step 1 - Set up the v2 Streaming API consumer](/vault-core/5-9/EN/reference/accounts/accounts_version_2#step_1_set_up_the_v2_streaming_api_consumer) for more information.
    

chat\_bubble

We recommend that you also modify Vault’s clients' stores to ensure that `event_id`s are stored for consumed v2 Account Events. This is because in the [v2 Accounts Streaming API](/vault-core/5-9/EN/api/core_api#account_events_v2_topic), there is a 1:n mapping of requests (used by the REST API clients) to events (consumed by Stream API clients). Events reconciliation uses the `event_id` on the messages sent to the Streaming API.
...
To make the v2 deployment ready for REST API calls to the `/v2/accounts` endpoint, see [Key differences between v1 and v2 Accounts API](/vault-core/5-9/EN/reference/accounts/accounts_version_2#key_differences_between_v1_and_v2_accounts).
...
Delete the deployment which is a client to the `/v1/accounts` endpoints. Then, create a new deployment which uses the synchronous `/v2/accounts` endpoint as explained in [Managing Accounts](/vault-core/5-9/EN/reference/accounts/accounts_version_2#managing_accounts).
...
## [](#step_6_use_v2account_migrations_for_bulk_account_conversions "Copy link to heading")Step 6 - Use v2/account-migrations for bulk Account conversions

Vault Core version 5.9 introduces support for bulk v2 Account conversions via the `v2/account-migrations` endpoint.

If you are using the `v1/account-migrations` endpoint for bulk Account conversions, you can continue to use this. However, Thought Machine recommends switching your endpoint to `v2/account-migrations` this optimises performance and failure remediation.
