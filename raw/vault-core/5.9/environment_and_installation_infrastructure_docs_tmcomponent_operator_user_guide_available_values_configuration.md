---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/tmcomponent_operator_user_guide/available_values_configuration"
title: "Available values configuration options"
scraped_at: "2026-06-17T04:58:42.302Z"
images: 0
---

# Available values configuration options

The following table shows the values configuration options available in Vault Core 5.9.
...
`audit.db.replica.host`

 | 

Hostname for the audit database read-only replica (if used).
...
`audit.db.replica.port`

 | 

Read-only replica database port to connect to.
...
`balances.db.replica.host`

 | 

Hostname for the balances database read-only replica (if used).
...
`ca_injector.ca_certs`

 | 

The CA cert(s) to be injected.
...
`common.csp_integrations.aws.iam_role_prefix`

 | 

An optional prefix which will be prepended to every AWS IAM role created by the installer results in a role name like <iam\_role\_prefix><digest(service\_account\_name)>. For example, given the prefix is "iam\_app" and the account name is "vault-account", the IAM role name will be "iam\_appe2897dbe404de1d4032d920048fdf99e"
...
`common.csp_integrations.enable_rbac_provisioning`

 | 

Used to specify whether RBAC resources for cloud service providers integrations must be provisoined for this installation. An example of cloud service providers integration is enablement of ASM on AWS. An example of RBAC resources in this case is AWS IAM roles and policies.
...
`common.db.aws_iam.append_username_suffix`

 | 

Controls whether the '\_iam' suffix is automatically appended to database usernames when AWS IAM authentication is enabled. Defaults to 'true' for backward compatibility. Set to 'false' to use custom usernames exactly as defined.
...
`common.object_store.encryption_key_name`

 | 

Path of the customer-managed encryption key (CMEK). If set, this key will be used to encrypt object store objects at rest, instead of the default CSP-provided keys.
...
```
arn:aws:kms:eu-west-2:123456789012:key/my-key
```
...
`core_apps.dlq_inspector.auth`

 | 

Configuration for the user authentication method for the DLQ Inspector application.
...
`core_apps.usage_monitor.auth`

 | 

Configuration for the user authentication method for the Usage Monitor application.
...
`core_apps.vault_accounts.auth`

 | 

Configuration for the user authentication method for the Vault Accounts application.

OIDC support for Vault Accounts App is introduced in Vault Core version 5.8. In 5.8, OIDC is automatically enabled in the presence of Single-Sign-On (SSO) configuration, defined in `bearer_auth.issuers`.

To prevent a breaking change, when this value is empty, e.g set to `null` or the value `~`, OIDC is selected in the presence of a SSO configuration. Otherwise, SAML is used.
...
`core_apps.vault_jobs.auth`

 | 

Configuration for the user authentication method for the Vault Jobs application.
...
`core_apps.vault_processing_groups.auth`

 | 

Configuration for the user authentication method for the Vault Processing Groups application.
...
`data_retention.db.migrator_user`

 | 

Data Retention DB migrator user used for running db migrations

Note: Hyphen characters as part of this value are supported, but recommended to be avoided. If you are using client certificates, set this value to be equal to the common name of the \\"data\_retention\_migrator\\" principal, which can be found in the DB principals artefact.
...
`data_retention.db.user`

 | 

Username to access the data retention database

Note: Hyphen characters as part of this value are supported, but recommended to be avoided. If you are using client certificates, set this value to be equal to the common name of the \\"data\_retention\\" principal, which can be found in the release.json file.
...
`edge_functions.db.migrator_user`

 | 

edge\_functions DB migrator user used for running db migrations Note: Hyphen characters as part of this value are supported, but recommended to be avoided. If you are using client certificates, set this value to be equal to the common name of the \\"edge\_functions\_migrator\\" principal, which can be found in the DB principals artefact.
...
`edge_functions.db.replica.host`

 | 

Hostname for the edge\_functions database read-only replica (if used).
...
`edge_functions.db.replica.port`
...
`ledger.jobs.balance_milestone_reconciler.account_verification_list`

 | 

If a comma-separated list of account IDs is provided, then the job will only verify the specified accounts. This option can only be used when read\_only is true. It cannot be used along with shard\_verification\_list.
...
```
account-id-1,account-id-2,account-id-5
```
...
`ledger.jobs.balance_milestone_reconciler.number_of_shards`

 | 

The number of shards to divide the account space into. A worker processes milestones one shard at a time. This number cannot change between runs unless all progress is reset.
...
`ledger.jobs.balance_milestone_reconciler.watermark_update_frequency`

 | 

The number of milestones to verify between updates of a watermark. If set to 1, a watermark will be updated after every milestone.
...
`ledger_balances.db.replica.host`

 | 

Hostname for the ledger\_balances database read-only replica (if used).
...
`ledger_balances.db.replica.port`
...
`postings.db.replica.host`

 | 

Hostname for the postings database read-only replica (if used).
...
`scheduler.migrator.cron`

 | 

This value controls how often the scheduler DB migration gets scheduled. It follows the cron syntax. Note: once finalise has been run for Vault version 5.9 or later, this job will still be scheduled but is a noop.
...
`scheduler.migrator.max_tps`

 | 

Setting this value to any value other than 0 will limit the number of writing queries run by the scheduler migrations per second. This is a global limit, so it will affect all migrations at once.
...
`scheduler.migrator.read_batch_size`

 | 

This value limits the number of rows to read at once by the scheduler migrations.
...
`scheduler.migrator.worker_pool_size`

 | 

This is the maximum number of concurrent requests that each of the migrators sends to the database.
...
`scheduler.migrator.write_batch_size`

 | 

This value limits the number of rows to add or update at once by the scheduler migrations.
...
`scheduler.single_db.enabled`

 | 

Toggles the source of truth used by Vault Core’s Scheduler. This config is true by default and should not be set to false unless advised by Thought Machine. - SET TO TRUE (Default): the Scheduler uses a single logical DB: scheduler. - SET TO FALSE: the Scheduler uses two logical DBs: scheduler and schedule\_manager. CRITICAL REQUIREMENTS FOR DISABLING (setting to false): - Must only be used in the case of a failed installation. - Setting this value to false after the installation of 5.9+ will prevent dependent features from functioning as expected, such as Bulk Account Conversions v2 and Sub-accounts.
...
`scheduler.single_db.legacy_writer.enabled`

 | 

Toggles Vault Core’s scheduler-legacy-data-writer, which ensures data is written to both the scheduler and schedule\_manager databases in case of a rollback installation. This config is true by default and should not be set to false unless advised by Thought Machine. - SET TO TRUE (Default): Ensures data safety by writing to both databases. Keep this enabled if you still need the ability to toggle 'scheduler.single\_db.enabled' back to false without losing data. - SET TO FALSE: Disables writes to the 'schedule\_manager' DB to reduce database load. WARNING: This is a permanent transition. CRITICAL REQUIREMENTS FOR DISABLING (setting to false): 1. 'scheduler.single\_db.enabled' MUST be set to true first. 2. Only disable after the Vault 5.9+ installer 'finalise' step has completed successfully. 3. Once set to false, this should never be toggled back to true.
...
Hostname for the vault read-only replica database
...
Database port to connect to for the read-only replica database.
...
`vault_jobs.db.replica.host`

 | 

Hostname for the vault\_jobs database read-only replica (if used).
