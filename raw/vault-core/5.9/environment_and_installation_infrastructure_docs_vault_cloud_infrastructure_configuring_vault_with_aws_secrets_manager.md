---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_vault_with_aws_secrets_manager"
title: "Configuring Vault Core with AWS Secrets Manager"
scraped_at: "2026-06-17T04:57:56.649Z"
images: 0
---
...
Vault Core 5.9 introduces a more deterministic naming convention for AWS IAM roles and policies. The naming logic has been updated to `{AWS_ROLE_PREFIX}{K8S_SERVICE_ACCOUNT_NAME}{K8S_SERVICE_ACCOUNT_NAME_CHECKSUM}`, which ensures resource names are predictable and compliant with AWS naming constraints.

In previous versions, resource names followed the pattern `{K8S_SERVICE_ACCOUNT_NAME}_{NAMESPACE}`, in which the `{NAMESPACE}` variable corresponded to the Kubernetes namespace where Vault Core was installed.

From Vault Core 5.9, the configuration details for AWS IAM roles and policies are as follows:

-   `{AWS_ROLE_PREFIX}`: defined within the `values.yaml` file, has a maximum limit of 32 characters
    
-   `{K8S_SERVICE_ACCOUNT_NAME}`: now truncated to the first 26 characters
    
-   `{K8S_SERVICE_ACCOUNT_NAME_CHECKSUM}`: truncated to the first six characters to ensure uniqueness while remaining within AWS character limits
    

### [](#creating_permission_boundaries "Copy link to heading")Creating Permission Boundaries
...
For more information about the purpose of the `tm_iam_prefix`, see the sections about [values.yaml](#valuesyaml) and [Vault Installer Operations](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_vault_with_aws_secrets_manager#vault_installer_operations).
...
When upgrading to Vault Core version 5.9 and above, ensure that the Vault Installer AWS policy allows `iam:TagRole` action. Failure to do this will cause an error during installation because the Vault Installer will try to create roles with tags.
...
### [](#creating_the_permissions_policies_and_vault_installer_role "Copy link to heading")Creating the permissions policies and Vault Installer role
...
### [](#configuring_the_values_yaml_file "Copy link to heading")Configuring the values.yaml file
...
-   `iam_proxy_ca_bundle`: Optional. If your HTTP proxy server decrypts and re-encrypts traffic sent through it, you must populate this field with the public CA trust chain that it uses to encrypt data. You must also configure the `ca-injector webhook` if your CA is used by other relevant parts of your infrastructure, for example databases. See [Configuring a custom certificate authority](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/tmcomponent_operator_user_guide/before_you_start#configuring_a_custom_certificate_authority) for more information. You must omit the “secrets\_management.hashicorp\_vault” values entirely.
...
For a guide, see: [Configuring certificate rotation with AWS Secrets Manager](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_vault_with_aws_secrets_manager#configuring_certificate_rotation_with_aws_secrets_manager)
...
In versions prior to Vault Core 5.9, a different format was used for the cert-rotation role name. If you are upgrading from versions prior to Vault Core 5.9 to any 5.9+ version and you wish to enable certificate rotation, you **must** create the new role described below to grant the necessary permissions. For more information on this new role name format, see: [Vault Installer role and policy](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_vault_with_aws_secrets_manager#vault_installer_role_and_policy).
...
These values are defined in the `values.yaml` file under: `secrets_management.aws_secrets_manager`
...
This value is defined in the `values.yaml` file under `common.csp_integrations.aws`:

-   `<iam_role_prefix>`: Optional, if provided will be added as a prefix to all AWS IAM role names used in Vault Core.
...
You must follow the steps in [Prerequisites](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_vault_with_aws_secrets_manager#prerequisites) before migrating any secrets.
