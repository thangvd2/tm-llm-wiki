---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/reference/edge_functions/overview_and_getting_started/triggers_overview"
title: "Triggers overview"
scraped_at: "2026-06-22T19:18:22.231Z"
images: 1
---
...
-   Automatically closing an account when its balances have reached zero. This is [used as an example in the tutorial](/vault-core/5-9/EN/tutorials/edge_functions_tutorials/how_to_create_a_trigger#example_auto_close).
    

## [](#triggers "Copy link to heading")Triggers

You can configure Edge Functions to execute upon an event in Vault Core. To do so, create an Edge Function Trigger that will route Vault Core events from a public streaming API Kafka topic to an Edge Function as execution requests.

An Edge Function Trigger is primarily defined as a combination of both:
...
-   the name of the source Kafka topic to process events from.
    ...
    Triggers can currently only process events from [Contract notification events](/vault-core/5-9/EN/api/core_api#contract_notification_events).
    

### [](#trigger_status "Copy link to heading")Trigger Status

A Trigger’s status controls the creation of new [Trigger Operations](/vault-core/5-9/EN/reference/edge_functions/overview_and_getting_started/triggers_overview#trigger_operations).
...
![Trigger Operation lifecycle](_assets/operation-lifecycle.DVeS80Vj_8S7Lj_vaultcor.svg)
...
-   A Trigger Operation with status `FAILED` or `FAILED_TRANSIENT` can be scheduled for a retried execution by calling the [`BulkRetryEdgeFunctionTriggerOperations` endpoint](/vault-core/5-9/EN/api/edge_functions_api#_edge_functions_api_v1_BulkRetryEdgeFunctionTriggerOperationsResponse_BulkRetryEdgeFunctionTriggerOperations). See [How to manually bulk retry Edge Function Trigger Operations](/vault-core/5-9/EN/tutorials/edge_functions_tutorials/how_to_manually_retry_triggered_operations) for more information about retrying failed operations.
...
`FAILED_TRANSIENT` results from an exception raised from an Edge Function that indicates a temporary failure. See [Implementing custom errors](/vault-core/5-9/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function#implementing_custom_errors).
...
To ensure optimal pipeline performance, Thought Machine recommends minimising the number of `ACTIVE` Edge Function Triggers. The total processing cost increases with the volume of events and the number of `ACTIVE` Triggers.
