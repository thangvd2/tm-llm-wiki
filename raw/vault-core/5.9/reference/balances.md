---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/reference/balances"
title: "Balances"
scraped_at: "2026-06-22T19:16:56.389Z"
images: 2
---
...
-   [Balances](/vault-core/5-9/EN/api/core_api#balances_version_1): Specify a time to see either the most recent state of an account balance, or the balance changes over a specified time period, based on value timestamps.
    
-   [Balances-v2](/vault-core/5-9/EN/api/core_api#balances_version_2): Specify a time to see either the most recent state of an account balance, the end state (including all future-dated Postings), or the balance changes over a specified time period, based on value or booking timestamps.
...
![A diagram showing the example use of balance coordinates in Vault Core](_assets/balance_coordinates.1pIu15lB_Zgl0oT_vaultcor.webp)
...
![A diagram showing an overview of the balance sheet from a bank’s perspective](_assets/t_side_overview.CV7jQh-k_ZkPuP_vaultcor.webp)

### [](#balance_initialisation "Copy link to heading")Balance initialisation

#### [](#accounts "Copy link to heading")Accounts

Vault Core accounts can be created via the Core API [v1/accounts](/vault-core/5-9/EN/api/core_api#accounts_version_1) endpoint or [v2/accounts](/vault-core/5-9/EN/api/core_api#accounts_version_2) endpoint (if you have completed the switch to v2/accounts) with an OPEN or PENDING status. For more information about account statuses:

-   For v1/accounts, see [Account statuses](/vault-core/5-9/EN/reference/accounts/accounts_version_1#account_statuses)
    
-   For v2/accounts, see [Account statuses](/vault-core/5-9/EN/reference/accounts/accounts_version_2#account_statuses)
    

When the balance of an OPEN account is affected for the first time (either by a query or by a posting made to it), balances are initialised synchronously using the [address details](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#AddressDetails) of the account’s associated product.
...
-   Customer Account’s [Current balances](/vault-core/5-9/EN/api/core_api#balanceslive_2): A balance as of now
    
-   Customer Account’s [End balances](/vault-core/5-9/EN/api/core_api#balancesend): A balance as of a point in time, which includes future-dated postings inserted before this time
    
-   [Time range](/vault-core/5-9/EN/api/core_api#balancesseries) of a Customer Account’s balances: Any changes to the balance over a specified time period
    

To retrieve Customer Account balances, call [GET /v2/balances/live](/vault-core/5-9/EN/api/core_api#_core_api_v2_balances_ListBalancesLiveResponse_ListBalancesLive), [GET /v2/balances/end](/vault-core/5-9/EN/api/core_api#_core_api_v2_balances_ListBalancesEndResponse_ListBalancesEnd) or [GET /v2/balances/series](/vault-core/5-9/EN/api/core_api#_core_api_v2_balances_ListBalancesSeriesResponse_ListBalancesSeries), providing the:
...
To retrieve ledger balances, call [GET /v1/ledger-balances](/vault-core/5-9/EN/api/core_api#_core_api_v1_ledger_balances_ListLedgerBalancesResponse_ListLedgerBalances), providing the:
...
To switch from the [AccountBalanceEvent](/vault-core/5-9/EN/api/core_api#accountbalanceevent) to the [BalanceEvent](/vault-core/5-9/EN/api/core_api#balanceevent) stream, you can use the `BalanceEvent.account_balance_sequence_number` to correlate with the `AccountBalanceEvent.related_resource.sequence_number`.

Use [event reconciliation](/vault-core/5-9/EN/api/core_api#event_reconciliation) to make sure you consumed all the messages from the `AccountBalanceEvent` when switching over.