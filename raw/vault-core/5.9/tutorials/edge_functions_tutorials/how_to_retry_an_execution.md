---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/tutorials/edge_functions_tutorials/how_to_retry_an_execution"
title: "How to retry an Edge Function execution"
scraped_at: "2026-06-22T19:19:46.765Z"
images: 0
---
...
There are a few considerations when implementing idempotency and ensuring you make and retry requests correctly. See [Advice on implementing idempotency](/vault-core/5-9/EN/tutorials/edge_functions_tutorials/how_to_execute_an_edge_function#advice_on_implementing_idempotency) for more information.

### [](#example_scenario "Copy link to heading")Example scenario

Take an Edge Function that orchestrates calls to open an account.

If the execution request fails, for example due to a temporary connection issue, then as long as you pass the same `idempotency_key` in the request, you can safely retry the same call to the [ExecuteEdgeFunction](/vault-core/5-9/EN/tutorials/edge_functions_tutorials/how_to_execute_an_edge_function#execute_an_edge_function) endpoint several times if necessary.
...
-   If multiple mutations occur against the same type of Vault Core API resource, then it is *required* that each mutation request have its own `request_id`. For information about how to implement this, see: [Deterministically generating request IDs](/vault-core/5-9/EN/tutorials/edge_functions_tutorials/how_to_retry_an_execution#deterministically_generating_request_ids)
...
-   Using [List LedgerBalances](/vault-core/5-9/EN/api/core_api#_core_api_v1_ledger_balances_ListLedgerBalancesResponse_ListLedgerBalances) to request a balance at a specific `ledger_timestamp` that remains consistent between retries
    
-   Using [List PostingInstructionBatches](/vault-core/5-9/EN/api/core_api#_core_api_v1_posting_instruction_batches_ListPostingInstructionBatchesResponse_ListPostingInstructionBatches) with specific timestamps for `start_time` and `end_time` that remain consistent between retries
...
-   meets the [uniqueness criteria outlined above](/vault-core/5-9/EN/tutorials/edge_functions_tutorials/how_to_retry_an_execution#uniqueness_of_idempotency_keys_and_request_ids)
...
All endpoints in the [Vault Core API](/vault-core/5-9/EN/api/core_api/) that mutate resources provide idempotency guarantees - that is: those that support `POST`/`PUT`/`DELETE` methods. It is important to make appropriate use of repeating the same parameters and the same request IDs for a retried execution that calls these endpoints.
...
-   [How to write an Edge Function](/vault-core/5-9/EN/tutorials/edge_functions_tutorials/how_to_write_an_edge_function)
...
-   [Edge Functions SDK reference](/vault-core/5-9/EN/reference/edge_functions/sdk_reference)
