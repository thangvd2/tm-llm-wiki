---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_disaster_recovery"
title: "Vault Disaster Recovery"
scraped_at: "2026-06-22T19:15:28.416Z"
images: 0
---
...
If you did not use the Deployment Operators to deploy Vault Core then you do not need to do this — for the next steps, see [Using the Disaster Recovery Operator](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_disaster_recovery#using_the_disaster_recovery_operator).
...
Run the [Disaster Recovery Operator instructions](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_disaster_recovery#disaster_recovery_operator_instructions) provided to:
...
A single DRPD can even define recovery steps for multiple services. We provide the DRPDs as part of the Vault release pack; they are separated by Vault subcomponent: Core, Edge Functions, Payments Hub, and XPL:
...
Core DRPD contains the steps to rehydrate relevant Kafka topics to Vault Core. Once this completes, Vault Core is ready to handle incoming traffic again. However, Core DRPD does not contain instructions to rerun Schedule Jobs, see [Scheduler](/vault-core/5-9/EN/reference/scheduler). This is handled in Schedule Jobs DRPD, for large deployments that may take a number of hours to complete. However, it is not a blocker for allowing traffic to Vault Core.

### [](#about_the_disaster_recovery_process_definitions "Copy link to heading")About the Disaster Recovery Process Definitions

The DRPDs and their associated [status conditions](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md#typical-status-properties) are managed by the DR Operator.

You can query the DRPDs currently in the Kubernetes by using the short names `drpd` / `drpds` (or their full name `disasterrecoverydisasterdefinition` / `disasterrecoverydisasterdefinitions`):

The top level status condition reason is printed as part of the information returned from the GET request. For example:

The individual status conditions are summarised in this table.

  
| Type | Reasons - Statuses | Description |
| --- | --- | --- |
| 
ResourcesClaimed
...
-   ClaimingInProgress - Unknown
    
-   ClaimingCompleted - True
...
We assign the DRPD as the non-managing owner of all the jobs referenced. This is to avoid having multiple DRPDs trying to reference and run the same job.
...
Tasks here refer to the collection of sequential steps that the DRPD is designed to execute. They include but are not limited to:

-   Updating config maps with the DR timestamps
    
-   Running specific jobs to bootstrap the environment for rehydration
    
-   Temporarily scaling stateful sets to foster a more efficient DR process
...
We perform cleanup resources that we temporarily scaled up during the DR process before the DRPD is considered complete.
...
This is a condition that only appears after the manual deletion process is initiated for the DRPD. It represents the restoration progress of the required resources. For more information, see [Deletion Behaviour](#_deletion_behaviour).
...
This is the top level condition type that represents the overall condition of a given DR process.

 |

#### [](#deletion_behaviour "Copy link to heading")Deletion Behaviour

When a DRPD is applied to the Kubernetes cluster, a specific Kubernetes finaliser is attached to the resource. When the deletion process is initiated manually, for example through `kubectl delete`, the finalisation logic executes before the finaliser is removed.

The main objective of this finalisation is to archive and reset the jobs previously activated by the DR Operator. Specifically, we set the parallelism of the job back to 0 and assign an annotation to the job metadata to reference the UID of the DRPD that last triggered it.

This ensures that during active-passive switchback testing, the jobs are ready to be rerun immediately, removing the need to rely on external operators to redeploy them after the DR process concludes.

You can observe the progression of the deletion process through the "ResourcesRestored" condition type.

### [](#downloadable_information "Copy link to heading")Downloadable information

You can download:

-   the `kernel-start.yaml` here: [Download](/vault-core/5-9/EN/resources/drpds/kernel-start.yaml)
    
-   the `edge-functions.yaml` here: [Download](/vault-core/5-9/EN/resources/drpds/edge-functions.yaml)
    
-   the `schedule-jobs.yaml` here: [Download](/vault-core/5-9/EN/resources/drpds/schedule-jobs.yaml)
    
-   the `payments-hub.yaml` here: [Download](/vault-core/5-9/EN/resources/drpds/payments-hub.yaml)
    
-   the `xpl.yaml` here: [Download](/vault-core/5-9/EN/resources/drpds/xpl.yaml)
...
The following instructions apply during disaster recovery of Vault Core in the Active-Passive deployment mode, after executing the `vaultctl dr promote-env` command. In this mode, Kafka rehydration is part of a multi-step process for promoting a passive environment. For more details, see [Active-Passive environment promotions](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/tmcomponent_operator_user_guide/active_passive#promoting_a_passive_environment).

The appropriate command flags that you must use depends on which [Kafka configuration](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/tmcomponent_operator_user_guide/active_passive#kafka) you are using.

Standalone Kafka Replicated Kafka

In an Active-Passive Vault Core deployment with a [Standalone Kafka configuration](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/tmcomponent_operator_user_guide/active_passive#kafka), `vaultctl` can infer the `from` and `to` timestamps automatically. The inferred `from` timestamp is derived from the maximum consumer lag of consumer groups in the active environment and the inferred `to` timestamp is the time of promotion. Use the following command to activate the Disaster Recovery Operator:

In an Active-Passive Vault Core deployment with a [Replicated Kafka configuration](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/tmcomponent_operator_user_guide/active_passive#kafka), you can infer the `to` timestamp automatically; however, you must provide the `from` timestamp. You must derive the `from` timestamp from the lag of the Kafka replicator service. It represents a timestamp before or equal to the original timestamp of the message that was most recently replicated from the active environment. You can approximate this value to an earlier timestamp to over rehydrate and ensure that no messages are missed; however, this will impact the recovery time. The inferred `to` timestamp is the time of promotion. Use the following command to activate the Disaster Recovery Operator:
...
For more information, see [Multiple physical databases](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/advanced_deployment_modes/multiple_physical_databases#disaster_recovery) in the Advanced deployment modes guidance.
