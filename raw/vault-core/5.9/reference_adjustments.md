---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/reference/adjustments"
title: "Adjustments"
scraped_at: "2026-06-17T05:03:03.598Z"
images: 18
---
...
-   This section assumes that you are familiar with several key Vault Core concepts, such as [Postings](/vault-core/5-9/EN/reference/postings), [Balances](/vault-core/5-9/EN/reference/balances), [Parameters](/vault-core/5-9/EN/reference/parameters), and [Flags](/vault-core/5-9/EN/reference/flags).
...
![Introduction to the purpose of Adjustments](_assets/adjustments_intro_pt1.ChKQltyf_KuCGa_vaultcor.svg)
...
![Introduction to the concept behind Adjustments](_assets/adjustments_intro_pt2.Bxxs0N2K_Z85qNc_vaultcor.svg)
...
The Adjustment process is invoked at recurring points in time known as [Adjustment points](/vault-core/5-9/EN/reference/adjustments#adjustment_points).

To create Adjustment points, you declare Smart Contract schedules ([SmartContractEventType](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#smartcontracteventtype)) as Adjustment-enabled, as described in the [Enabling Adjustments](/vault-core/5-9/EN/reference/adjustments#step_1_set_up_the_smart_contract) section.
...
1.  [Detect](/vault-core/5-9/EN/reference/adjustments#adjustment_detection): Vault Core scans for any backdated events which could affect historic decisions made by the Account’s Smart Contract. If no events are detected, the process completes here.
    
    lightbulb
    
    Vault Core optimises the detection process to avoid too many false positives.
    
2.  [Compute](/vault-core/5-9/EN/reference/adjustments#adjustment_computation): Vault Core computes any impact on the Account’s balances, calculating the difference between the current balance time series and the adjusted balance time series. If there is no impact on balances, the process completes here.
    
3.  [Correct](/vault-core/5-9/EN/reference/adjustments#adjustment_correction): Vault Core applies a correction posting to each affected Adjustment point, correcting the Account’s balance position at these points.
    

![A summary of the Adjustment process](_assets/adjustment_process.COrEDbZJ_1zKNWc_vaultcor.svg)
...
![Introduction to Adjustment points](_assets/adjustment_points.BudHm2Us_ZqzPPz_vaultcor.svg)
...
-   Any inserted *Posting Instruction Batch* with a `value_timestamp` set to a point before any [supported Smart Contract hook](/vault-core/5-9/EN/reference/adjustments#supported_hooks) executions took place
    
-   Any inserted *Flag* with an `effective_timestamp` set to a point before any [supported Smart Contract hook](/vault-core/5-9/EN/reference/adjustments#supported_hooks) executions took place
...
![An example of Vault Core setting the Adjustment timeline](_assets/setting_adjustment_timeline.BDzlucfz_ZJl_vaultcor.svg)
...
With the new timeline of events, Vault Core simulates each Smart Contract hook execution (including any previously run Adjustment schedule hooks) and hook results.
...
![An Adjustment timeline that crosses over previous Adjustments](_assets/adjustment_timeline_spanning.C8FWGjNk_Z1_vaultcor.svg)

In this example, event A led to an Adjustment correction at the previous Adjustment point 1, and event C has since been backdated to before this point. When an Adjustment is triggered at Adjustment point 2, Vault Core recalculates the new chain of events (A > C > B), running the schedule at Adjustment point 1 again.

#### [](#events_during_the_adjustment_computation_time "Copy link to heading")Events during the Adjustment computation time

When the current Adjustment point is reached (after the `timeline_end_timestamp`) and an Adjustment computation runs, any subsequent events inserted and/or backdated are not factored into this computation (since it cannot yet see them), and are instead picked up at the next Adjustment point. This is represented in the following diagram by events B and C:

![Showing events during an Adjustment calculation](_assets/adjustment_calculation_time.Bwf3w_Fv_Z1S_vaultcor.svg)
...
![Showing Vault Core applying correction postings](_assets/corrections.BbNA58D8_27nx3L_vaultcor.svg)

Vault Core then runs the schedule at the current Adjustment point.

warning

**Adjustments correct only the balance position, and only at Adjustment points**. There are a number of important considerations as a result of this. For more information, see [Adjustment correction constraints](/vault-core/5-9/EN/reference/adjustments#adjustment_correction_constraints).
...
Reruns the original `conversion_hook` executions, and includes the detected backdates. Depends on the versioning policy as per [Product versioning in Adjustment computations](/vault-core/5-9/EN/reference/adjustments#product_versioning_in_adjustment_computations).

 |
| 

`conversion_adjustment_hook`

 | 

Overrides the Business As Usual (BAU) logic on the `conversion_hook` executions. Optional.
...
Adjustment hooks cannot define their own decorators; these are inherited from the corresponding BAU hook.
...
The Adjustment hooks are passed both `snapshot_vault` and `adjusted_vault` arguments:
...
The `contract_update_timestamp` used when the Account was converted. You can call [v2 Accounts](/vault-core/5-9/EN/api/core_api#accounts_version_2) to find out the most recent timestamp used. Depends on the Adjustment versioning policy as per [Product versioning in Adjustment computations](/vault-core/5-9/EN/reference/adjustments#product_versioning_in_adjustment_computations).
...
The latest of the job’s `schedule_timestamp` or the `minimum_observation_timestamp` set via a Schedule Group or Processing Group
...
## [](#product_versioning_in_adjustment_computations "Copy link to heading")Product versioning in Adjustment computations

From Vault Core 5.9, you can configure [Product versions](/vault-core/5-9/EN/api/core_api#productversion) to define a historic *Adjustments versioning policy*, enabling Adjustment computations to reflect the Smart Contract versions used by an Account in BAU along the Adjustment timeline. You can also customise the behaviour of conversion hooks when running Adjustments with a historic versioning policy.

To ensure compatibility with Adjustments, and to avoid Adjustments re-triggering bugs in historic Product versions, you can patch these versions with substitute Smart Contract versions exclusively for use in Adjustment computations.

### [](#product_versioning_options "Copy link to heading")Product versioning options

There are two options for the use of Product versions when triggering Adjustments:

-   Historic (recommended): Use the Smart Contract versions that were associated with an Account at the time
    
-   Latest (default): Use only the most current Smart Contract version, irrespective of which versions were associated with an Account at the time
    

#### [](#historic_versioning_policy "Copy link to heading")Historic versioning policy

When the historic Adjustments versioning policy is defined, Adjustment computations accurately reflect the adjusted Account’s Product (Smart Contract) version history along the Adjustment timeline, including:

-   Associated Smart Contract hook code for [supported hooks](/vault-core/5-9/EN/reference/adjustments#supported_hooks) and data fetcher requirements (such as `expected_parameters`)
    
-   [conversion\_hook](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/smart_contracts_api_reference4xx/hooks#conversion_hook) executions for any conversions that occurred along the Adjustment timeline
    

For example, if an Account was associated with Product version A between T0 and T4, then Product version B from T4 to now, and the Adjustment timeline runs from T2 to T7:

-   Any event between T2 and T4 is re-evaluated using Product version A
    
-   Any event from T4 onwards is re-evaluated using Product version B, including the conversion from Product version A (at T4)
    

![Adjustment computation with Product versioning](_assets/computations_for_versions_historic.CUweC_vaultcor.svg)

#### [](#latest_versioning_policy_default "Copy link to heading")Latest versioning policy (default)

When the Adjustments versioning policy is omitted (or set to "latest"), the Adjustments computation uses only the *latest* Product (Smart Contract) version over the Adjustment timeline, irrespective of the Smart Contract versions used by the Account in BAU.

For example, if an Account was associated with Product version A between T0 and T4, then Product version B from T4 to now, and the Adjustment timeline runs from T2 to T7, then any event between T2 and T4 is re-evaluated as if the Account was *always* using Product version B:

![Adjustment computation with Product versioning](_assets/computations_for_versions_latest.DCU7E8k_vaultcor.svg)

### [](#effect_of_product_versioning_on_parameter_value_backdates "Copy link to heading")Effect of Product versioning on Parameter value backdates

#### [](#historic_versioning_policy_2 "Copy link to heading")Historic versioning policy

When the historic Adjustments versioning policy is defined, Vault Core detects backdated Parameter values referred to by any Product (Smart Contract) version in the Adjustment timeline, provided the Core API `ExpectedParameter` value is effective over a period of time where the Account was associated with the relevant Product versions.

For example, if an Account was associated with Product versions A then B (current), and `ExpectedParameter` P1 is included in version A but not B:

-   *Scenario 1*: A backdated P1 Parameter value change, effective at time T2, is in scope for an Adjustment, because the Account was on Product version A at the effective time
    
-   *Scenario 2*: A backdated P1 Parameter value change, effective at time T5, is *not* in scope for an Adjustment, because the Account is on Product Version B at the effective time
    

![Parameter value backdates across contract versions](_assets/parameter_backdates_for_versions_histori_vaultcor.svg)

Conversely, if an Account was associated with Product versions A then B (current), and `ExpectedParameter` P1 is included in version B but not A:

-   *Scenario*: A backdated P1 Parameter value change, effective at time T2, is in scope for an Adjustment, because P1 comes into effect at T4 with the conversion to Product version B (the conversion now needs to be aware of the backdated change)
...
Currently, with the historic Adjustments versioning policy defined, Vault Core does not detect backdated Parameter value changes if the `ExpectedParameter` is not present in the *latest* Smart Contract version. The resolution of TM-124600, as summarised in the [Vault Core 5.9 release notes](/vault-core/5-9/EN/vault_release_information/technical_details_of_this_release#adjustments_2), aims to remove this limitation.

#### [](#latest_versioning_policy_default_2 "Copy link to heading")Latest versioning policy (default)

When the Adjustments versioning policy is omitted (or set to "latest"), Vault Core can only detect backdated Parameter values if the *latest* Product Version includes this Core API `ExpectedParameter`.

For example, if an Account was associated with Product versions A then B (current), and `ExpectedParameter` P1 is included in version A but not B:

-   *Scenario*: A backdated P1 Parameter value change, effective at time T2, is not in scope for an Adjustment, because even though the `ExpectedParameter` was included in Product version A, Vault Core treats T2 as if the Account was associated with Product version B (where this `ExpectedParameter` is *not* included)
    

![Parameter value backdates across contract versions](_assets/parameter_backdates_for_versions_latest._vaultcor.svg)

Conversely, if an Account was associated with Product versions A then B (current), and `ExpectedParameter` P1 is included in version B but not A:

-   *Scenario*: A backdated P1 Parameter value change, effective at time T2, is in scope for an Adjustment, because even though the `ExpectedParameter` was *not* included in Product version A, Vault Core treats T2 as if the Account was associated with Product version B (where this `ExpectedParameter` *is* included)
    

![Parameter value backdates across contract versions](_assets/parameter_backdates_for_versions_latest__vaultcor.svg)

### [](#setting_the_historic_adjustments_versioning_policy "Copy link to heading")Setting the historic Adjustments versioning policy

lightbulb

For backwards compatibility, Adjustments uses only the latest Product version (the latest Adjustments-enabled Smart Contract version) by default.

Thought Machine recommends checking and [patching Product versions](/vault-core/5-9/EN/reference/adjustments#patching_product_versions) as required, then setting the historic Adjustments versioning policy.

You can set the `adjustments_versioning_policy` to `HISTORIC` when creating or updating a Product version via the Core API, so that Adjustments use historic Smart Contract versions:

-   When creating a Product version, call [POST /v1/product-versions](/vault-core/5-9/EN/api/core_api#_core_api_v1_products_ProductVersion_CreateProductVersion), setting the `adjustments_versioning_policy` to `ADJUSTMENTS_VERSIONING_POLICY_HISTORIC`.
    
    Example create request to enable historic Product versioning
    
-   When updating a Product version, call [POST /v1/product-versions:batchUpdate](/vault-core/5-9/EN/api/core_api#_core_api_v1_products_BatchUpdateProductVersionsResponse_BatchUpdateProductVersions), setting the `adjustments_versioning_policy` to `ADJUSTMENTS_VERSIONING_POLICY_HISTORIC`.
    
    Example update request to enable historic Product versioning
    

chat\_bubble

The default state of Adjustments (with no policy set) is the equivalent of setting the `adjustments_versioning_policy` to `ADJUSTMENTS_VERSIONING_POLICY_LATEST`.

### [](#patching_product_versions "Copy link to heading")Patching Product versions

You can substitute one Smart Contract version for another, so that the substitute is used exclusively in Adjustment computations. You achieve this by patching Product versions, and it is useful for:

-   Ensuring errors that originally occurred in BAU are not re-triggered in Adjustments
    
-   Making historic Product versions compatible with Adjustments
    

Before setting the `adjustments_versioning_policy` to `HISTORIC`, Thought Machine recommends reviewing historic Product versions and patching them to ensure that the relevant Smart Contract code respects Adjustments requirements and [constraints](/vault-core/5-9/EN/reference/adjustments#constraints_when_using_adjustments), such as the types of [fetchers](/vault-core/5-9/EN/reference/adjustments#smart_contract_requirement_fetching_constraints) used on [supported hooks](/vault-core/5-9/EN/reference/adjustments#supported_hooks).

chat\_bubble

Patching cannot be used to:

-   Retrospectively enable Adjustments; or
    
-   Change the BAU behaviour of a Smart Contract (it only affects the rerunning of hooks during Adjustment computation)
    

For more information, see [Patching constraints](/vault-core/5-9/EN/reference/adjustments#patching_constraints).

lightbulb

While you cannot currently configure correction points for Product versions, support for flexible Adjustment corrections aims to address this. For more information, refer to IMP-1774 on the [Vault Core roadmap](/vault-core/latest/EN/product_documents/product_roadmaps).

#### [](#patching_historic_product_versions "Copy link to heading")Patching historic Product versions

You can patch previous Product versions to fix bugs or make these versions compatible with Adjustments:

1.  Call [POST /v1/product-versions](/vault-core/5-9/EN/api/core_api#_core_api_v1_products_ProductVersion_CreateProductVersion) to upload any new version(s) containing the changes you want to apply in the patch
    
2.  Call [POST /v1/product-versions:batchUpdate](/vault-core/5-9/EN/api/core_api#_core_api_v1_products_BatchUpdateProductVersionsResponse_BatchUpdateProductVersions) to apply the patch version to the required historic Product version(s)
    

chat\_bubble

You can also use a current, active Product version to patch a previous Product version.

#### [](#patching_the_latest_product_version "Copy link to heading")Patching the latest Product version

You can patch the latest Product version to fix bugs or change the versioning policy, for example. To patch Product version A with fix version B:

1.  Convert the relevant Accounts to the new Product version B, so that ongoing BAU hook executions are corrected
    
2.  Patch Product version A with Product version B, so that any Adjustments covering the period where the Accounts were on Product version A do not re-trigger the issue
    

#### [](#how_to_patch_a_product_version "Copy link to heading")How to patch a Product version

chat\_bubble

This tutorial assumes you are aware of the [patching constraints](/vault-core/5-9/EN/reference/adjustments#patching_constraints).

You can only patch existing Product versions. To patch a Product version, call [POST /v1/product-versions:batchUpdate](/vault-core/5-9/EN/api/core_api#_core_api_v1_products_BatchUpdateProductVersionsResponse_BatchUpdateProductVersions), providing the:

-   `product_version.id` of the Product version you want to apply the patch to
    
-   `product_version.adjustments_patch_version_id` of the Product version you want to apply the patch from - this is used during Adjustments, in lieu of the Product version used in BAU
    
-   `adjustments_patch_version_id` as a value in `update_mask.paths[]`
    

For example, if Product version A is patched by Product version B, set `product_version.id` to `A` and `product_version.adjustments_patch_version_id` to `B`.

##### [](#example_patch_product_version_request "Copy link to heading")Example patch Product version request

The following example request patches both Product version A and Product version B so that Adjustment computations instead use Product version C:

The response confirms the `adjustments_patch_version_id` applicable to Product versions A and B for Adjustments purposes.
...
| --- | --- | --- |
| 
The Product version has been updated to use the new patch version for Adjustments purposes.

 | 

[ProductVersionEvent](/vault-core/5-9/EN/api/core_api#productversionevents)(ProductVersionUpdatedEvent)

 | 

`vault.core_api.v1.products.product_version.events`

 |

#### [](#how_adjustments_use_product_version_patches "Copy link to heading")How Adjustments use Product version patches

To factor in any new Product version patches, Vault Core first checks the Account’s current Product version’s `adjustments_patch_version_id` *when an Adjustment is processed* (not at the effective time of the triggering schedule).

Vault Core then checks the `adjustments_versioning_policy`:

-   If set to `ADJUSTMENTS_VERSIONING_POLICY_LATEST` (also the default behaviour if omitted), Vault Core uses the patch version ID for the latest Product version only
    
-   If set to `ADJUSTMENTS_VERSIONING_POLICY_HISTORIC`, Vault Core uses any patch version IDs found in all historic Product versions in the Adjustment timeline
    

#### [](#retrieving_patch_ids "Copy link to heading")Retrieving patch ids

To identify which Product Versions are patched by specific versions, call [GET /v1/product-versions](/vault-core/5-9/EN/api/core_api#_core_api_v1_products_ListProductVersionsResponse_ListProductVersions) providing the `adjustments_patch_version_ids`. For example, to find all versions patched by Product Version 'B', provide 'B' as the value for `adjustments_patch_version_ids`.

lightbulb

You can optionally provide a `snapshot_timestamp`, to return the `adjustments_patch_version_ids` of the Product version(s) used at the specified time.

### [](#customising_conversion_behaviour "Copy link to heading")Customising conversion behaviour

With the historic Adjustments versioning policy, Vault Core reruns any original `conversion_hook` executions.

To configure Adjustment-specific logic you can use the `conversion_adjustment_hook`. This is exposed to both the original data, excluding backdates, and the adjusted data, including backdates. For more information, see the [Adjustment hook arguments](/vault-core/5-9/EN/reference/adjustments#the_adjustment_hook_arguments) section.

info

-   If a conversion is rejected during an Adjustment, the Adjustment will error. [Patch](/vault-core/5-9/EN/reference/adjustments#patching_product_versions) the relevant Smart Contracts to address the rejection reason and retry the Adjustment.
    
-   If your Smart Contracts use [rejections](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#rejection) in conversion hooks, Thought Machine recommends configuring the `conversion_adjustment_hook` to avoid raising rejections during Adjustments.
    

Example use of the conversion adjustment hook

The following is an example use of the `conversion_adjustment_hook`. In this case the Account receives a £100 fixed bonus if the balance is greater than or equal to £1000 at the conversion time:

-   If the bonus was paid previously (due to the balance at conversion being >= £1000), the bonus is honoured regardless of retroactive backdates.
    
-   If the bonus was not paid previously but a new backdate means that it should have been paid, pay the bonus retroactively as the result of the backdate.
...
-   Aware of the additional [Constraints when using Adjustments](/vault-core/5-9/EN/reference/adjustments#constraints_when_using_adjustments)
    
-   Using v2/accounts and the associated `vault.core_api.v2.accounts.account.events` topic (whether you have integrated anew with these, or have completed [switching from v1 to v2 Accounts API](/vault-core/5-9/EN/reference/accounts/switching_from_v1_to_v2_accounts_api))
    
-   Using Contracts Language version 4 (CLv4) Smart Contracts
    
-   Only using Core API [Parameters](/vault-core/5-9/EN/reference/parameters) and the accompanying `expected_parameters` syntax in your Smart Contract
...
For more complete examples, see the [Latest Product Library release package](/vault-core/5-9/EN/product_library/release_information/downloads).
...
Call [PUT /v2/accounts/{account.id}](/vault-core/5-9/EN/api/core_api#_core_api_v2_accounts_Account_UpdateAccount) to convert your Customer Accounts to the new Smart Contract version. From this point forwards, the initial Adjustment timeline will begin when the first Adjustment is triggered.

chat\_bubble

From Vault Core 5.9, you can opt for Adjustments to use the Account’s history of Product (Smart Contract) versions over the Adjustment timeline. For more information see [Product versioning in Adjustment computations](/vault-core/5-9/EN/reference/adjustments#product_versioning_in_adjustment_computations).
...
Adjustments reuse existing [Smart Contract testing](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/development_and_testing) approaches, with the exception of Simulation, which is not currently supported.
...
As explained in the Smart Contract constraints, the [Simulation endpoint ignores any Adjustments functionality](/vault-core/5-9/EN/reference/adjustments#smart_contract_simulation_does_not_support_adjustments), so it cannot be used to test Adjustments.

### [](#end_to_end_testing "Copy link to heading")End-to-end testing

As Adjustments are triggered by schedules, you can use Vault Core’s [Accelerated Testing](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/development_and_testing#accelerated_testing) approach to test Adjustments in an end-to-end manner. Thought Machine recommends familiarising yourself with the overall concept before reading on.

#### [](#test_setup "Copy link to heading")Test setup

Accelerated Testing requires all events to be backdated, including account creation. Providing that the events are processed in the intended order, none of them will overlap with existing hook executions, and thus none will lead to [detected events](/vault-core/5-9/EN/reference/adjustments#adjustment_detection). This allows you to perform a backdated test setup without accidentally requiring Adjustments.
...
![Adjustment test setup](_assets/test_setup.GIrfX-Qd_Z1szlea_vaultcor.svg)
...
![Adjustment test](_assets/test_adjustment.Cop4HA0I_1NXuzL_vaultcor.svg)
...
-   `previous_watermark_timestamp`: Since the `previous_watermark_timestamp` is equal to the `timeline_end_timestamp` of the account’s previous Adjustment, this field is also affected.
...
![Adjustment test caveats](_assets/test_caveats.BkPRUoJT_1ij7xh_vaultcor.svg)
...
The [Vault Jobs](/vault-core/5-9/EN/reference/core_apps_and_operations_dashboard/vault_jobs) App provides insight into Adjustments progress via the schedule that triggered the Adjustment:
...
When an Adjustment is in `STATUS_ERROR`, you can retry it via the [/v1/adjustments:bulkRetry](/vault-core/5-9/EN/api/core_api#_core_api_v1_adjustments_BulkRetryAdjustmentsResponse_BulkRetryAdjustments) endpoint.

chat\_bubble

Republishing a failed job via [/v1/jobs:batchRepublish](/vault-core/5-9/EN/api/core_api#_google_protobuf_Empty_BatchRepublishJobs) when the associated Adjustment is in `STATUS_ERROR` will not cause the Adjustment to be retried.

While the Adjustment is in `STATUS_ERROR`, the associated schedule will be stuck. As schedules on Adjustment-enabled Smart Contracts [must all be in the same group](/vault-core/5-9/EN/reference/adjustments#schedules_must_be_in_a_single_schedule_group), this means all affected accounts' schedules will be stuck. If the root cause of the error is not transient in nature (retries do not resolve the error), the following approaches can help address this:

1.  If the Adjustments error is caused by a Smart Contract issue, you can fix the bug and retry the errored Adjustments. For example:
    
    1.  If the latest Smart Contract version contains the issue, [patch](/vault-core/5-9/EN/reference/adjustments#patching_the_latest_product_version) the affected Accounts with a fixed Smart Contract version and retry the errored Adjustments.
        
    2.  If the Adjustment is using `ADJUSTMENTS_VERSIONING_POLICY_HISTORIC`, a historic Smart Contract version used in the Adjustment may fail Adjustment compatibility validation at runtime (for example, due to unsupported fetching behaviour in a hook that is re-run during Adjustments). In this case, patch the affected historic Product version and retry the errored Adjustment.
...
3.  Republishing via [/v1/jobs:batchRepublish](/vault-core/5-9/EN/api/core_api#_google_protobuf_Empty_BatchRepublishJobs) with status `JOB_STATUS_OVERRIDDEN` will unblock subsequent schedules, however:
    
    1.  As per point 2, any corresponding Adjustments will error until the original errored Adjustment is addressed.
        
    2.  If the job and Adjustment were expected to produce directives, see [Manual Corrections](/vault-core/5-9/EN/reference/adjustments#manual_corrections) for guidance on submitting these postings manually.
...
#### [](#posting_instruction_level_timestamps_are_incompatible_with_adjustments "Copy link to heading")Posting-Instruction-level timestamps are incompatible with Adjustments
...
You should only use PIB-level value and booking timestamps for Adjustments. This is because when timestamps are set at PI-level, Vault Core considers only the PIB’s `insertion_timestamp`, which means that:

-   Backdating a Posting at PI-level does not trigger Adjustment computations
    
-   For the purpose of Adjustment computations, all PIs in a PIB are considered as valued and booked at the PIB’s `insertion_timestamp`
    

As a result, Posting-Instruction-level timestamps cause Adjustments to behave erroneously regarding corrections.

#### [](#custom_posting_instructions_are_incompatible_with_adjustments "Copy link to heading")Custom Posting Instructions are incompatible with Adjustments

Adjustments do not currently support the use of [Custom](/vault-core/5-9/EN/reference/postings#custom_chainable_but_only_to_other_custom_types) Posting Instructions sent by client integrations. Adjustments only supports the use of these Posting Instructions within Smart Contracts.

Custom Posting Instructions sent by client integrations cause Adjustments to error.

#### [](#back_booking_in_isolation_is_not_detected "Copy link to heading")Back-booking in isolation is not detected

Adjustments do not detect solely back-booked PIBs. Unless the PIB is also back-valued, it is not detected as requiring Adjustment.

chat\_bubble

There are also constraints around the booking timestamps of Adjustment correction Postings. For more information, see [Correction Posting timestamps](/vault-core/5-9/EN/reference/adjustments#correction_posting_timestamps).

#### [](#restrictions_are_not_considered_in_adjustments "Copy link to heading")Restrictions are not considered in Adjustments

Restrictions are not factored into Adjustments. This does not affect previous decisions on Postings because `pre_posting_hook` decisions are not re-evaluated, but it can affect contract-generated Postings that were originally rejected by a restriction and are not rejected in Adjustments. For this reason Thought Machine recommends always using `override_all_restrictions=True` on Smart Contract Posting Instructions.
...
This includes [Account Attributes](/vault-core/5-9/EN/reference/accounts/account_attributes) and derived parameter values that depend on balances; you should only request these using the Adjustment point effective times as the `effective_timestamps` values for [GET /v1/account-attribute-values](/vault-core/5-9/EN/api/core_api#_core_api_v1_account_attributes_ListAccountAttributeValuesResponse_ListAccountAttributeValues) (or `instance_param_vals_effective_timestamp` value for [GET /v1/accounts/{id}](/vault-core/5-9/EN/api/core_api#_core_api_v1_accounts_Account_GetAccount)) as each relevant Adjustment’s `timeline_end_timestamp`.
...
When performing Adjustments, Vault Core re-evaluates the `post_posting_hook`, `post_parameter_change_hook`, and `scheduled_event_hook`. Whether or not Vault Core re-evaluates the `conversion_hook` depends on how you are using [Product versioning in Adjustment computations](/vault-core/5-9/EN/reference/adjustments#product_versioning_in_adjustment_computations):

-   If you set the Adjustments versioning policy to `HISTORIC`, then Vault Core re-evaluates the `conversion_hook`.
    
-   If you set the Adjustments versioning policy to `LATEST` (which is also the default if not set), then Vault Core does not re-evaluate the `conversion_hook` because Adjustments only use the latest Smart Contract version (even if the Adjustment timeline spans a conversion from a previous Adjustment-enabled Smart Contract version) - therefore Adjustments over a timeline where a conversion made Postings do not re-evaluate these Postings.
    

The following hooks are *not* re-evaluated by Vault Core when performing an Adjustment:
...
-   Ignores Schedule Updates when calculating an Adjustment (instead Vault Core re-runs schedules as they ran in BAU)
    
-   Does not send notifications or update schedules when applying corrections
    

Since Schedule Updates (`UpdateAccountEventTypeDirective` s) are not factored in when performing Adjustments, this means that Adjustments for Smart Contracts relying on amending schedules do not behave as expected, because Vault Core ignores Schedule Updates when calculating an Adjustment and when applying any corrections.
...
When using any of the [supported hooks](/vault-core/5-9/EN/reference/adjustments#supported_hooks), Smart Contracts enabling Adjustments *must not*:
...
As per the [integration constraint](/vault-core/5-9/EN/reference/adjustments#balances_should_only_be_queried_at_adjustment_point_times), Smart Contract [Account data fetchers](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/concepts#account_data_fetchers) should only fetch balances using the Adjustment point effective times.
...
Smart Contract Simulation does not support Adjustments functionality. Adjustment-enabled Smart Contracts can run BAU functionality in Simulation, but any Adjustments functionality is ignored. For more information, see [Unsupported features](/vault-core/5-9/EN/reference/contracts/contract_simulation#unsupported_features).

#### [](#smart_contract_versions_must_remain_compatible_with_adjustments "Copy link to heading")Smart Contract versions must remain compatible with Adjustments

To preserve intended BAU product logic across an Account’s Adjustment timeline, it is important to maintain compatibility across Adjustment-enabled Smart Contract versions backing that Account, while considering the Adjustments versioning policy. The following table provides some example considerations:

<table class="tableblock frame-all grid-all stretch"><colgroup><col style="width: 30%;"> <col style="width: 25%;"> <col style="width: 45%;"></colgroup><tbody><tr><td class="tableblock halign-left valign-top"><p class="tableblock">Change</p></td><td class="tableblock halign-left valign-top"><p class="tableblock">Adjustments versioning policy</p></td><td class="tableblock halign-left valign-top"><p class="tableblock">Outcome</p></td></tr><tr><td class="tableblock halign-left valign-top" rowspan="2"><p class="tableblock">Removing the <code>post_posting_hook</code> from the latest Smart Contract version.</p></td><td class="tableblock halign-left valign-top"><p class="tableblock"><code>HISTORIC</code></p></td><td class="tableblock halign-left valign-top"><p class="tableblock">There are no <code>post_posting_hook</code> executions on the latest Smart Contract version, but Adjustments can still re-evaluate <code>post_posting_hook</code> executions that occurred while the Account was on previous Smart Contract Versions.</p></td></tr><tr><td class="tableblock halign-left valign-top"><p class="tableblock"><code>LATEST</code></p></td><td class="tableblock halign-left valign-top"><p class="tableblock">Adjustments cannot re-evaluate <em>any</em> <code>post_posting_hook</code> executions, including those that originally occurred while the Account was on any previous Smart Contract Versions which defined the <code>post_posting_hook</code>.</p></td></tr><tr><td class="tableblock halign-left valign-top" rowspan="2"><p class="tableblock">Removing an <code>ExpectedParameter</code> and associated logic from the latest Smart Contract version.</p></td><td class="tableblock halign-left valign-top"><p class="tableblock"><code>HISTORIC</code></p></td><td class="tableblock halign-left valign-top"><p class="tableblock">There are no <code>post_parameter_change_hook</code> executions on the latest Smart Contract version, but Adjustments can still re-evaluate <code>post_parameter_change_hook</code> executions that occurred while the Account was on previous Smart Contract Versions.</p></td></tr><tr><td class="tableblock halign-left valign-top"><p class="tableblock"><code>LATEST</code></p></td><td class="tableblock halign-left valign-top"><p class="tableblock">Adjustments cannot use this logic when re-evaluating <em>any</em> <code>post_parameter_change_hook</code> executions, including those that originally occurred while the Account was on a previous Smart Contract Version which defined the <code>ExpectedParameter</code>.</p></td></tr><tr><td class="tableblock halign-left valign-top" rowspan="2"><p class="tableblock">Defining <a href="/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#smartcontracteventtype">SmartContractEventTypes</a> in the latest Smart Contract version which do not have equivalents in previous versions.</p></td><td class="tableblock halign-left valign-top"><p class="tableblock"><code>HISTORIC</code></p></td><td class="tableblock halign-left valign-top"><p class="tableblock">Adjustments re-evaluate <code>scheduled_event_hook</code> executions for the event types in each Smart Contract version.</p></td></tr><tr><td class="tableblock halign-left valign-top"><p class="tableblock"><code>LATEST</code></p></td><td class="tableblock halign-left valign-top"><p class="tableblock">Adjustments only re-evaluate <code>scheduled_event_hook</code> executions for the event types in the latest Smart Contract version, and ignore all <code>scheduled_event_hook</code> executions for the event types in previous Smart Contract versions.</p></td></tr></tbody></table>
...
[PostingInstructionEnrichment](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#postinginstructionenrichment) objects are defined in the `pre_posting_hook` - therefore they are not re-evaluated during Adjustments and should not be relied upon.

#### [](#avoid_using_posting_instruction_level_timestamps "Copy link to heading")Avoid using Posting-Instruction-level timestamps

As stated in [Integration constraints](/vault-core/5-9/EN/reference/adjustments#integration_constraints), you should only use PIB-level value and booking timestamps for Adjustments. This guidance also applies to Smart Contracts.

#### [](#schedules_must_be_in_a_single_schedule_group "Copy link to heading")Schedules must be in a single Schedule Group

To ensure that Adjustments do not race with other schedules, Adjustment-enabled Smart Contracts may only use a single schedule group, which must contain all schedules (Adjustment and non-Adjustment). As schedule job events in the Adjustment timeline are sequenced by the latest of the job’s `schedule_timestamp` or `minimum_observation_timestamp`, the BAU order is replicated in Adjustments.
...
There are also constraints as a result of Smart Contracts that initiate Postings to Internal Accounts, as described [below](/vault-core/5-9/EN/reference/adjustments#avoid_asymmetric_postings_if_your_smart_contract_posts_to_internal_accounts).

Smart Contracts that initiate Postings to other Customer Accounts, or between any Accounts that are not being adjusted, are not supported because they cause Adjustments to behave erroneously regarding corrections.
...
-   [Do not instruct Postings to other Customer Accounts](/vault-core/5-9/EN/reference/adjustments#do_not_instruct_postings_to_other_customer_accounts)
    
-   [Do not instruct Postings between Accounts that are not being adjusted](/vault-core/5-9/EN/reference/adjustments#do_not_instruct_postings_between_accounts_that_are_not_being_adjusted)
    
-   [Avoid asymmetric Postings if your Smart Contract posts to Internal Accounts](/vault-core/5-9/EN/reference/adjustments#avoid_asymmetric_postings_if_your_smart_contract_posts_to_internal_accounts)
...
For example, if an Adjustment-enabled Smart Contract uses [Account Attribute](/vault-core/5-9/EN/reference/accounts/account_attributes) calculations for a non-commutative product feature, and `GET /v1/account-attribute-values` is called with `effective_timestamps` between two Adjustment points, this may return incorrect balances.

lightbulb

Thought Machine recommends running Account Attribute (or derived parameter) calculations using the same timestamp value as each relevant Adjustment’s `timeline_end_timestamp`, as explained in [Balances should only be queried at Adjustment point times](/vault-core/5-9/EN/reference/adjustments#balances_should_only_be_queried_at_adjustment_point_times).
...
In the case of asymmetric transaction logic, 'reversing' a transaction by backdating a Posting to mirror the original transaction may not fully reverse the side-effects, because the Adjustment only calculates the effect of the backdated Posting itself. Instead, investigate using [supported Adjustment hooks](/vault-core/5-9/EN/reference/adjustments#supported_hooks) to achieve the desired outcome.
...
### [](#patching_constraints "Copy link to heading")Patching constraints

Constraints when patching Smart Contract versions as part of [Product versioning in Adjustment computations](/vault-core/5-9/EN/reference/adjustments#product_versioning_in_adjustment_computations).

#### [](#core_api_patching_constraints "Copy link to heading")Core API patching constraints

The following constraints apply to patching via Core API:

-   Patching does not trigger any Adjustments.
    
-   Patching does not trigger the conversion hook.
    
-   Patching only permits changes to the following ProductVersion fields via Core API:
...
For simpler tracing of the current Product Version patches in use, patches cannot be chained, which manifests in two ways:

-   A Product Version that is patched cannot patch another Product Version. For example, if B is patched by C (B → C), A cannot be patched by B. This would create the chain A → B → C.
    
-   A Product Version that patches another Product Version cannot be patched itself. For example, if B patches A (A → B), B cannot be patched by C. This would also create the chain A → B → C.
    

#### [](#smart_contract_patching_constraints "Copy link to heading")Smart Contract patching constraints

Patching only permits changes to the following Smart Contract code:

-   Any [Account Fetcher Requirements](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/smart_contracts_api_reference4xx/account_fetcher_requirements)
...
-   Patching cannot affect `expected_parameters`
    
-   You cannot add schedules or hooks in patches
    
-   You cannot retrospectively apply changes to Product business logic in patches (for example, you cannot retrospectively enable Adjustments)
...
All correction Postings made as a result of Adjustments have their:
...
To understand more about a given Adjustment’s outcomes, see [Adjustment explainability](/vault-core/5-9/EN/reference/adjustments#adjustment_explainability).

#### [](#correction_timing_constraints "Copy link to heading")Correction timing constraints

It is quite common for scheduled processes to decouple the point in time at which they run (operational time), and the point in time from which they use data (business time). For example, an End of Day process might run a schedule at 02:00 AM, but use account balances as at midnight.

Vault Core triggers an Adjustment (and makes resulting correction Postings) at the Adjustment point(s), and before the `scheduled_event_hook` for the Adjustment-enabled schedule runs. Therefore, if the schedule itself uses a balance cut-off time prior to the Adjustment point, the schedule does not factor in the correction Posting. This is illustrated with an End-of-Day (EoD) example in the following diagram:

![Showing the correction timing constraint of Adjustments](_assets/correction_timing_constraint.CHZMoh3J_9B_vaultcor.svg)
...
As per the [Smart Contract constraint](/vault-core/5-9/EN/reference/adjustments#do_not_instruct_postings_to_other_customer_accounts), Adjustments does not support correction Postings to other Customer Accounts.