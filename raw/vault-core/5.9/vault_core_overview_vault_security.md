---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/vault_core_overview/vault_security"
title: "Security"
scraped_at: "2026-06-17T04:57:08.846Z"
images: 5
---
...
![Thought Machine SaaS Responsibility Model](_assets/saas-responsibility-model.BhrSX3TC_1CnV2_vaultcor.webp)
...
![Thought Machine certification logos](_assets/compliance.B8_n8N9w_1zn9hD_vaultcor.webp)
...
Thought Machine publishes all identified and resolved vulnerabilities for all supported versions of Vault Core to clients via its [Vulnerability Report](/vault-core/5-9/EN/vault_release_information/vulnerability_report). The details for each vulnerability include:
...
![Diagram demonstrating Vault Core SAML integration with common identity providers](_assets/interactive-access.DYrdBTeJ_ZtHiM5_vaultcor.webp)
...
![Diagram demonstrating programmatic access to Vault Core APIs](_assets/programmatic-access.C2wv8xMM_Z1EfXlC_vaultcor.webp)
...
-   Using attribute-based access control (an Extension which only applies to the [Vault Accounts App](/vault-core/5-9/EN/reference/core_apps_and_operations_dashboard/vault_accounts))
...
Policies must be pre-configured and referenced in [Workflow and Ticket](/vault-core/5-9/EN/reference/policies/legacy-policies#workflow-and-ticket-policies) definitions in order for access to be controlled within instances of those Workflows or Tickets. To enforce access controls via policies, an access control context is passed into requests to the Workflow API, to confer the specified level of access to the targeted resource.
...
As of Vault Core 5.8, you can use attribute-based access control logic to set up Product-based Access Control for the Vault Accounts App. For more information, see [Setting up and configuring OIDC authentication](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/setting_up_and_configuring_oidc_authentication).

### [](#opa_policies "Copy link to heading")OPA Policies

Vault Core uses the [Open Policy Agent](https://www.openpolicyagent.org/) to evaluate policies and enforce access control. These Policies can be created and updated using the `/v1/policies` endpoints on the Core API. For more information, see the [Core API](/vault-core/5-9/EN/api/core_api#policy_management) documentation.
...
For more detail on how to work with Rego Policies and a list of the schemas provided by Vault Core, see [OPA Policies](/vault-core/5-9/EN/reference/policies/opa-policies#overview_opa).
