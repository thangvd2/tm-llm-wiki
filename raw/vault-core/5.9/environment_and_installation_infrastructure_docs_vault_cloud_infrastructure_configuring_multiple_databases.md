---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases"
title: "Configuring multiple databases"
scraped_at: "2026-06-17T04:58:00.180Z"
images: 0
---
...
From Vault Core 5.5, you can choose to distribute the logical databases required by the system across multiple physical database clusters. Vault Core can run with up to three physical databases, each one housing different logical databases used by different Vault Core services. Here, we are referring to primary databases, not read replicas.

A multi-database topology allows you to:

-   Optimise your deployment so it can support a larger amount of data than you can currently store within a single database.
    
-   Gain additional performance benefits when compared with the use of an individual database.
    

## [](#supported_database_topologies "Copy link to heading")Supported database topologies

Vault Core supports the following database topologies:

-   All logical databases (Vault Core, Activity, and Warm Storage data) on a single physical database. This is the default deployment option.
    
-   Vault Core and Activity data on one physical database, with Warm Storage data on a second physical database.
    
-   Vault Core and Warm Storage data on one physical database, with Activity data on a second physical database.
    
-   Vault Core data on one physical database, Warm Storage data on a second physical database, and Activity data on a third physical database.
    

For all database topologies, you can optionally have a read replica with any of the physical databases to improve performance under specific workloads. See [Configuring read replicas for performance](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/using_a_relational_database#configuring_read_replicas_for_performance) for more information.
...
The Activity database stores data for [audit log](/vault-core/5-9/EN/api/audit_api#audit_logs), [action log](/vault-core/5-9/EN/api/audit_api#action_logs), and [contract events](/vault-core/5-9/EN/api/core_api#contract_events). It serves the following public APIs:
...
-   **Option 3:** If your Account volumes exceed 30 million and you plan to perform [event reconciliation](/vault-core/5-9/EN/api/core_api#event_reconciliation) against events from the [Vault Core Streaming API](/vault-core/5-9/EN/api/core_api#core_streaming_api), you should also consider additionally configuring and using a dedicated read replica for the Vault Core database. See [Configuring read replicas for performance](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/using_a_relational_database#configuring_read_replicas_for_performance).
...
Using multiple databases has implications for disaster recovery - for more information, see the [Vault Disaster Recovery guide](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_disaster_recovery) and [Using multiple physical databases](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/advanced_deployment_modes/multiple_physical_databases).
...
Must point to the Warm Storage database if it is a separate physical database.
...
Optional value: This should only be populated if you are using a separate physical Warm Storage database with a read replica.
...
This is distinct from the `db.host` setting and should only be used if you intend to set up additional databases for read replication. The schema is initialised for hosts assigned to this parameter.
...
Must point to the Activity database if it is a separate physical database.
...
The following sections cover the process of migration, but you should also refer to [Background to the disaster recovery process for Vault Core with multiple physical databases](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/advanced_deployment_modes/multiple_physical_databases#background_to_the_disaster_recovery_process_for_vault_core_with_multiple_physical_databases).

You can find information about making changes to the host settings in the [values.yaml](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#values_yaml_settings) section of this guide; however, there are some additional precautions to take.
...
1.  Pause affected Vault Core services by [scaling down the relevant Kafka consumers](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#scaling_services_during_migration).
...
2.  Clone the database instance:
    ...
    3.  [Query the API](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#testing_the_api) to verify functionality.
...
2.  Pause the affected Vault Core services by [scaling down the relevant Kafka consumers](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#scaling_services_during_migration).
...
4.  Switch database configuration by updating `values.yaml` to point to the new database - see [related values.yaml settings](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#values_yaml_settings).
    
5.  Reinstall/update Vault Core and [query the API](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#testing_the_api) to verify access to the new database.
...
2.  Revert the database configuration to its original state:
    ...
    2.  [Query the API](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#testing_the_api) to verify access to the original database.
...
1.  Pause Vault Core services:
    ...
    2.  Pause the other Vault Core [services](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#services_2).
        
    3.  Place Vault Core into [Passive mode](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#passive_mode).
...
2.  Revert and reinstall database configuration:
    ...
    1.  Revert any database renaming:
        ...
        See [Disabling the old database](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#disabling_the_old_database).
        
    2.  Reinstall original database configuration via the [values.yaml settings](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#values_yaml_settings) - point Warm Storage to the original database.
...
4.  Initiate replaying Ledger and Account events for Warm Storage:
    ...
    1.  Scale up the [Disaster Recovery Operator](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_disaster_recovery#disaster_recovery_operator_instructions).
        
        You must [scale down the TMComponent and crown operators](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_disaster_recovery#scaling_down_the_tmcomponent_and_crown_operators) if it is in use.
        
    2.  Run the Disaster Recovery rehydration process for Warm Storage - see [Using the Disaster Recovery Operator](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_disaster_recovery#using_the_disaster_recovery_operator).
        
        For an example Disaster Recovery Process Definition (DRPD) for topics consumed by Warm Storage, see [Example Warm Storage DRPD](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/advanced_deployment_modes/multiple_physical_databases#example_warm_storage_drpd).
...
2.  Revert and reinstall database configuration:
    ...
    1.  Revert any database renaming - see [Disabling the old database](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#disabling_the_old_database).
        
    2.  Update `values.yaml` to point to the new database.
        
    3.  Reinstall/update Vault Core.
        
    4.  [Query the API](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#testing_the_api) to verify functionality.
        
    
3.  Re-enable the affected Vault Core services by scaling up the relevant Kafka consumers.
    
4.  Rewind the Kafka consumer offsets of the [relevant consumers](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/vault_cloud_infrastructure/configuring_multiple_databases#scaling_services_during_migration) to hydrate data written during the migration. Contact your Thought Machine representative for help with this.
    

## [](#configuring_snapshots "Copy link to heading")Configuring snapshots

You should consider using Point-in-Time-Recovery and snapshot frequency, which are methods to restore a new database instance in the event of a disaster. For more information, see [Background to the disaster recovery process for Vault Core with multiple physical databases](/vault-core/5-9/EN/environment_and_installation/infrastructure_docs/advanced_deployment_modes/multiple_physical_databases#background_to_the_disaster_recovery_process_for_vault_core_with_multiple_physical_databases).