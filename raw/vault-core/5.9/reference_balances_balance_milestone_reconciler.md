---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/reference/balances/balance_milestone_reconciler"
title: "Balance Milestone Reconciler"
scraped_at: "2026-06-17T05:01:12.297Z"
images: 6
---
...
Milestone Reconciler is a Kubernetes CronJob that iteratively verifies that the balance milestones value credits and debits are identical to a corresponding sum of postings credits and debits written to the Ledger. Incorrect milestones are reported via logs and an alert. If not in the [read-only mode](/vault-core/5-9/EN/reference/balances/balance_milestone_reconciler#read_only_mode), the Reconciler can be halted and resumed later on without losing progress. The job manages reconciliation progress by dividing the whole set of customer accounts into smaller sets called shards. It processes these shards concurrently.
...
In the [Grafana dashboard](/vault-core/5-9/EN/reference/balances/balance_milestone_reconciler#monitoring), incorrect milestones are classified as either *incorrect derived* or *incorrect non-derived*:
...
![grafana config](_assets/grafana_config.C_kxP0XG_mBAKk_vaultcor.webp)
...
![grafana verification details](_assets/grafana_verification_details.Dzeyb5pg_Zz_vaultcor.webp)
...
![grafana progress overview](_assets/grafana_progress_overview.B5TaiHWx_120hU_vaultcor.webp)
...
![grafana performance](_assets/grafana_performance.DWnqVsn7_ZIrObl_vaultcor.webp)
...
![grafana db query latency](_assets/grafana_db_query_latency.Bb1v7dbo_286OYy_vaultcor.webp)
