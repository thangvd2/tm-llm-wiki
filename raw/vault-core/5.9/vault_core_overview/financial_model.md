---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/vault_core_overview/financial_model"
title: "Financial model"
scraped_at: "2026-06-22T19:12:57.115Z"
images: 4
---

# Financial model

The Vault Core financial model defines how Vault Core operates financial movements (including [postings](/vault-core/5-9/EN/vault_core_overview/financial_model#postings_in_the_vault_core_financial_model) and the [End of Day (EOD)](/vault-core/5-9/EN/vault_core_overview/financial_model#end_of_day) process) in order to derive an account balance at any point in time.
...
The [Postings](/vault-core/5-9/EN/reference/postings) resource is the sole mechanism for moving funds between two accounts in Vault Core. Postings can be generated internally (for example, by a [scheduled job](/vault-core/5-9/EN/reference/scheduler#schedule_jobs)) or externally by a client using a [Core API](/vault-core/5-9/EN/api/overview) resource.
...
-   To satisfy all use cases, Vault Core requires [three timestamps](/vault-core/5-9/EN/reference/postings#postings_timestamps) on each posting. This enables Vault Core to provide three different balance timeseries (or optionally orderings) for the same set of postings.
...
*End of Day* (EOD) is the *position* of all of a bank’s accounts at the end of a banking day. It is also used to refer to the [process](/vault-core/5-9/EN/vault_core_overview/financial_model#end_of_day_process) by which the [EOD position](/vault-core/5-9/EN/vault_core_overview/financial_model#reaching_an_eod_position) is reached (for example, by taking each customer’s balance at exactly midnight and applying the agreed interest or fees).

### [](#end_of_day_process "Copy link to heading")End of Day process

Running a successful End of Day (EOD) process requires a clear understanding of the key terms steps involved to determine the [EOD position](/vault-core/5-9/EN/vault_core_overview/financial_model#reaching_an_eod_position) or balances of all accounts.

The diagram below demonstrates the relationships between the terms used in the EOD process:

![eod-terms.png](_assets/eod-terms.BtrrWyBH_kctUB_vaultcor.webp)

*Business as usual* (BAU): The movement of funds ([postings](/vault-core/5-9/EN/api/core_api#postings)) that occur during normal customer use of their account, such as an ATM withdrawal or a card purchase. This normal customer activity occurs uninterrupted, even as Vault Core calculates interest and fees on their account.
...
If there are any postings that require backdating after the 2º cut-off, you must use a different approach to determine the EOD position—for example, by simulating a hypothetical account lifecycle. For more information, see [Contract Simulation API](/vault-core/5-9/EN/api/core_api#contracts).
...
*Completion*: The point in time at which all EOD processing has completed. This is important for downstream systems, as it indicates the point at which the [EOD position](/vault-core/5-9/EN/vault_core_overview/financial_model#reaching_an_eod_position) is available for onward reporting.
...
![eod-pos-1.png](_assets/eod-pos-1.BTPDIB1G_ZPtorf_vaultcor.webp)
...
![eod-pos-2.png](_assets/eod-pos-2.DmddAStq_2fu30_vaultcor.webp)
...
Through Vault Core, you can use [Smart Contracts](/vault-core/5-9/EN/reference/contracts) to specify Schedule Code, a component of Smart Contracts that is executed on a regular cadence. Schedule Code can refer to the state of the account from a specific point in time. The Smart Contract can make any necessary calculations to generate accrued interest and fees, while adding the corresponding set of postings to the ledger (marking these postings so that they are clearly differentiated downstream for reporting purposes).

A number of Vault Core resources are available to implement EOD processing:

-   *Schedule(s)*: The rule(s) defining the cadence for repeated generation of a schedule event. For more information, see [Schedules](/vault-core/5-9/EN/reference/scheduler#schedules).
    
-   *Schedule events*: The triggers that Smart Contracts listen to in order to run their Schedule Code. For more information, see [Scheduler Jobs](/vault-core/5-9/EN/reference/scheduler#schedule_jobs).
    
-   *Schedule code*: The code within a Smart Contract that executes when a specific schedule event occurs. The Code takes the effective date of the Schedule Event to determine the state of the account to base calculations. For more information, see [Smart Contracts scheduled event hook](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/smart_contracts_api_reference4xx/hooks#scheduled_event_hook).
    
-   *Schedule tags*: All accounts must finish processing all their Schedule Code before the EOD position is known. A Schedule Tag is used for this purpose; Vault Core issues an event when all tagged processing has completed. For more information, see [account schedule tags](/vault-core/5-9/EN/api/core_api#account_schedule_tags).
    
-   *Operation Event*: The event issued when Vault Core has finished executing all the Schedule Code in all accounts with a shared account schedule tag. For more information, see [Core Streaming API Scheduler events](/vault-core/5-9/EN/api/core_api#scheduler_events).
    

lightbulb

You can also view long-running processes via the [Vault Jobs](/vault-core/5-9/EN/reference/core_apps_and_operations_dashboard/vault_jobs) app.

### [](#typical_implementation "Copy link to heading")Typical implementation

Banks implementing an End of Day (EOD) process will be consumers of the data produced by Vault Core. All the events that Vault Core produces should be collected, and used to populate and maintain an [Online](/vault-core/5-9/EN/vault_core_overview/coexistence#online_data_hub) or [Offline](/vault-core/5-9/EN/vault_core_overview/coexistence#offline_data_hub) Data Hub.
...
-   Overnight postings are generated between the 2º cut-off point and Completion:
    ...
    -   The bank can decide which day to include the overnight postings (as described in [Reaching an EOD position](/vault-core/5-9/EN/vault_core_overview/financial_model#reaching_an_eod_position)).
        
    -   An [account schedule tag](/vault-core/5-9/EN/api/core_api#account_schedule_tags) can be used to indicate when all postings have been generated.
        
    

The diagram below exemplifies an implementation for EOD processing:

![eod-typical-implementation.png](_assets/eod-typical-implementation.DcJ_p6nB_2oCQ_vaultcor.webp)
...
5.  [OperationEvents](/vault-core/5-9/EN/api/core_api#operationevent) indicates the completion of a Schedule Tag. This can trigger the final round of reconciliation before calculating the EOD position.
