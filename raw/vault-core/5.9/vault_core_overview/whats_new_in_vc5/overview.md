---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/vault_core_overview/whats_new_in_vc5/overview"
title: "Overview of changes"
scraped_at: "2026-06-22T19:13:11.180Z"
images: 4
---
...
Accounts will support the use of [Processing Groups](/vault-core/5-9/EN/reference/processing_groups), which allow for the isolation of Postings and additional controls over associated schedules.
...
You can use the Expected Parameters syntax introduced in Smart Contracts language version 4 (CLv4), and therefore you can use the new [Parameters](/vault-core/5-9/EN/reference/parameters) resource. You can also use [the Parameter Value Hierarchy](/vault-core/5-9/EN/reference/parameters/building_and_managing_the_parameter_value_hierarchy); introduced in Vault Core 5.3.
...
-   For a detailed explanation of the v2 Account statuses and their behaviour, see [Account statuses](/vault-core/5-9/EN/reference/accounts/accounts_version_2#account_statuses).
    
-   There is nuance in the way that statuses operate when performing a switch from the v1 to v2 services. For the complete process, see [Switching from v1 to v2 Accounts API](/vault-core/5-9/EN/reference/accounts/switching_from_v1_to_v2_accounts_api).
...
It is not possible to override Restrictions on the v2/Accounts API (or any of the [Usable v1 Accounts endpoints after the switch](/vault-core/5-9/EN/reference/accounts/switching_from_v1_to_v2_accounts_api#usable_v1_accounts_endpoints_after_the_switch)).

## [](#parameters_changes "Copy link to heading")Parameters changes

This section explains the behavioural changes that apply to Parameters in Vault Core 5, and the changes that then apply when you [switch to using the Core API Parameters](/vault-core/5-9/EN/reference/parameters/switching_to_core_api_parameters) resource.
...
Account-owned and Globally-owned Parameter values are managed independently of Smart Contracts, via a single API (or the CLU), defined in the Contracts as [Expected Parameters](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/concepts#expected_parameters).
...
Parameters apply inheritance to values - a globally-owned value is superseded by [Parameter Value Hierarchy](/vault-core/5-9/EN/reference/parameters/building_and_managing_the_parameter_value_hierarchy) values (which themselves follow an internal order), and ultimately by Account-owned values.
...
When the switch to the Core API Parameters resource is complete, you can no longer use `POST /v1/account-migrations` and `POST /v1/account-update-batches` for Account conversions. Instead you must use `PUT /v2/accounts/{account.id}`, or `POST /v2/account-migrations` for batched Account conversions.
...
Smart Contracts that are not opted in will trigger the `post_parameter_change_hook` for the creation of Account-owned Parameter Values only (not the expiry of Account-owned values). If you include `triggers_post_parameter_change_hook` for any `ExpectedParameter` in a CLv4 Smart Contract, you must also define both `triggers_pre_parameter_change_hook` and `triggers_post_parameter_change_hook` (as either true or false) for ALL of the `expected_parameters` in that CLv4 Smart Contract.
...
Since the new Account-owned Parameter Values introduced in Vault Core 5 have no concept of 'optional', and since we have introduced a minimum string length of 1, calling `/v1/parameter-values POST` (when you have completed [switching to the Core API Parameters resource](/vault-core/5-9/EN/reference/parameters/switching_to_core_api_parameters)) and attempting to set an empty string will produce an error. Instead, you need to set the `effective_to_timestamp`, as explained in [Unsetting a Parameter Value](/vault-core/5-9/EN/reference/parameters/using_core_api_parameters#unsetting_a_parameter_value).
...
-   All Instance Parameter values created throughout an Account’s lifetime (including before Vault Core 5) will be available via `v1/parameter-values` when you complete [switching to the Core API Parameters resource](/vault-core/5-9/EN/reference/parameters/switching_to_core_api_parameters).
...
To learn about the type (shape) mapping, see [Parameter shape to constraint mapping](/vault-core/5-9/EN/vault_core_overview/whats_new_in_vc5/overview#parameter_shape_to_constraint_mapping).
...
-   All existing Global and Instance Parameters have been recreated in the new Parameter system, and are available via the new `v1/parameters` and `v1/parameter-values` endpoints (ready for when you [switch to the Core API Parameters resource](/vault-core/5-9/EN/reference/parameters/switching_to_core_api_parameters))
    
-   All existing APIs used to define Global and Instance Parameters (and their values) continue to work until their official removal. Any Parameters and values defined via these APIs are automatically recreated in the [Parameters](/vault-core/5-9/EN/api/core_api#parameters) resource (following the mapping rules in this section), and are available via the Parameter API endpoints
...
To create equivalents of legacy parameters in the new [Parameters API](/vault-core/5-9/EN/api/core_api#parameters), the upgrade to Vault Core 5 maps the existing [Global Parameters](/vault-core/5-9/EN/api/core_api#global_parameters) and Smart Contract Instance Parameters. These are both mapped using the `id` of Global Parameters and the `name` of instance Parameters, together forming as a new Core API Parameter `id`.
...
![Vault Core mapped Instance Parameters.svg](_assets/mapped_instance_parameters.BeI42y6I_Z1hn_vaultcor.svg)
...
-   Thought Machine recommends that you do not create new Instance Parameters on Vault Core 5 before you understand how the mapping works. This mapping affects any Core API Parameters created automatically, which will be visible when [Switching to Core API Parameters](/vault-core/5-9/EN/reference/parameters/switching_to_core_api_parameters).
...
![Vault Core mapped Global Parameters.svg](_assets/mapped_global_parameters.DgkQ3q5p_Hpjjx_vaultcor.svg)
...
For more information see [Retrieving Parameters and their values](/vault-core/5-9/EN/reference/parameters/using_core_api_parameters#retrieving_parameters_and_their_values)
...
-   In some cases, Parameters which have the same Shape but different attributes can still lead to different Core API Parameters, as described in [Attributes which affect mapping](/vault-core/5-9/EN/vault_core_overview/whats_new_in_vc5/overview#attributes_which_affect_mapping).
...
-   No two Posting Instructions may share the same `value_timestamp` or `booking_timestamp`. However, from Vault Core 5.9 onwards, [Multiple Posting Instructions](/vault-core/5-9/EN/reference/postings#multiple_posting_instructions) within a single client transaction may share the same `value_timestamp` or `booking_timestamp`, providing that they are in different Posting Instruction Batches.
...
1.  Set the PG timezone through an API call to the [Processing Groups API](/vault-core/5-9/EN/api/core_api#processinggroups).
    
    info
    
    Make sure that you first read the information described in [Processing Groups](/vault-core/5-9/EN/reference/processing_groups). If you consider that using the feature is suitable for your requirements and you wish to implement a PG, make sure that you follow and complete the accompanying instructions exactly.
...
For more information about monitoring `EnrichedPostingInstructionBatchEvent`, see: [Core Streaming API Posting Events](/vault-core/5-9/EN/api/core_api#posting_events)

![timezone-adjust-postings.png](_assets/timezone-adjust-postings.DDhEZ9Fz_ZvFrk4_vaultcor.webp)
