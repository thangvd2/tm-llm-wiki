---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/product_library/release_information/previous_release_notes/2024-23_release_notes"
title: "2024-23 Release Notes"
scraped_at: "2026-06-22T19:20:07.989Z"
images: 0
---

# 2024-23 Release Notes

These release notes describe the features and defect fixes delivered as part of the 2024-23 Product Library Release and provide technical guidance on using it. They are intended for all clients of Thought Machine who consume the Product Library and are interested in taking this release.

The release notes reflect the components included in the Product Library Release 2024-23. Thought Machine has designed and built the included Products to demonstrate configurability of financial products and processes within Vault Core. The Products are intended solely as aids to accelerate understanding and development of Smart Contracts, Workflows and our Platform. Use is at your sole discretion and risk.
...
Thought Machine has designed and tested this release against Vault Core 5.4 and Configuration Layer Utility 5.1.0; elements of the library may not work with other versions.
...
Upgrade Inception SDK to support ASv2 Simulation

 | 

Adds support for V2 accounts simulation instructions to the Inception SDK, allowing simulation tests written using the Inception SDK to create and update V2 accounts.
...
Make the Inception Renderer Compatible with Features as Python package imports

 | 

Adds renderer support for user-built Python packages containing smart contract code.
...
Improve use of Processing-Group Endpoint for Schedule Testing

 | 

The e2e test framework no longer systematically updates the processing group’s status to paused and then active when updating the minimum observation timestamp in e2e schedule tests. This improves test speed and reliability.
...
Add SDK Structure / Change Scope Documentation

 | 

Adds documentation to better understand how to improve the SDK in response to new Vault Core features.
...
Improve Library guidance for CLU and existing ASv1 internal accounts

 | 

Adds guidance to internal account CLU resource files to help deal with conflicts caused by existing internal accounts (e.g. existing ASV1 internal account with same id as updated ASv2 internal account).
...
Improve one-of model validators

 | 

Introduces a single implementation for pydantic 'one-of' model validators, reducing maintenance burden.
...
Improve mypy settings

 | 

Improves mypy config to avoid false negatives due to `follow_imports=skip` and false positives from pydantic models.
...
Address Credit Card Type Hint issues with Transaction Types

 | 

Improves type hints on Credit Card affecting transaction-type arguments.
...
Loan incorrectly treats accrual as non-EMI when accrual and due amount schedules are aligned

 | 

Clarifies the logic around determining if interest is emi/non-emi via comprehensive code comments.
...
Renderer fails to get file checksum from commit on merge commits

 | 

Addresses a renderer bug triggered by merge commits. This also removes the renderer dependency on pydriller.
...
Add-on-only fields are not optional in pydantic models

 | 

Ensures SDK Vault Core models don’t require fields that are only used by add-on features.
...
Credit Card Test Flake for value\_datetime set in future

 | 

Avoids explicitly setting value timestamps unnecessarily in Credit Card tests, which could accidentally future-date postings and cause errors if the test machine’s clock is ahead of the Vault Core test environment’s clock.
...
The renderer does not allow global\_parameter metadata to reference features, meaning the global parameter names must be hardcoded in each contract template.
