---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-9/EN/reference/parameters/using_core_api_parameters"
title: "Using Core API Parameters"
scraped_at: "2026-06-17T05:03:16.674Z"
images: 0
---
...
If you have an existing Parameters integration or have recently upgraded to Vault Core 5, see the overview of [Parameters changes](/vault-core/5-9/EN/vault_core_overview/whats_new_in_vc5/overview#parameters_changes) for more information about how the Core API Parameters system is different from the legacy Smart Contract INSTANCE, TEMPLATE and GLOBAL parameters.
...
To create a [Parameter](/vault-core/5-9/EN/api/core_api#parameter), call `POST /v1/parameters`, providing the:
...
-   Parameter type and any declared constraints as part of `parameter.constraint`
    ...
    For more information about Parameter types and examples of their use, see [Supported Parameter types](/vault-core/5-9/EN/reference/parameters/using_core_api_parameters#supported_parameter_types).
...
[ParameterEvent](/vault-core/5-9/EN/api/core_api#parameter_events)(ParameterCreatedEvent)
...
You can either create a single [Parameter Value](/vault-core/5-9/EN/api/core_api#parametervalue) for a single owner, or create an atomic batch of values for a single owner.
...
-   Value(s) of the Parameter(s) (satisfying any constraints set when [creating a Parameter](/vault-core/5-9/EN/reference/parameters/using_core_api_parameters#creating_a_parameter)) in `parameter_value.value`
...
Assuming that the associated Smart Contract’s [pre\_parameter\_change\_hook](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/smart_contracts_api_reference4xx/hooks#pre_parameter_change_hook) accepted the change (or the hook was skipped), the response confirms the change to the value.

lightbulb

To learn about the behaviour of Smart Contract Parameter hooks, see [Smart Contract Parameter hook behaviour](/vault-core/5-9/EN/reference/parameters/using_core_api_parameters#smart_contract_parameter_hook_behaviour).
...
-   You can also create an account-owned Parameter value when [creating an Account](/vault-core/5-9/EN/reference/accounts/accounts_version_2#creating_an_account).
    
-   To learn about the fields you can set for Parameter types, see the [Parameter Value](/vault-core/5-9/EN/api/core_api#parametervalue) API documentation.
...
[ParameterValueEvent](/vault-core/5-9/EN/api/core_api#parametervalueevent)(ParameterValueUpdatedEvent)
...
You can also retrieve details of *effective* parameter values, either for a [Parameter Value Hierarchy Node](/vault-core/5-9/EN/reference/parameters/building_and_managing_the_parameter_value_hierarchy#retrieving_effective_parameter_values_for_a_node), or for a [Customer Account](/vault-core/5-9/EN/reference/accounts/accounts_version_2#retrieving_effective_parameter_values_for_a_customer_account).

#### [](#retrieving_parameter_ids "Copy link to heading")Retrieving Parameter IDs

1.  Call the [GET /v1/product-versions:batchGet](/vault-core/5-9/EN/api/core_api#productversion), specifying the Product version(s) you want to retrieve Parameter IDs for.
...
3.  To retrieve the IDs of Global Parameters, call [GET /v1/global-parameters:batchGet](/vault-core/5-9/EN/api/core_api#globalparameter).
...
To retrieve a filtered and paginated list of [ParameterValues](/vault-core/5-9/EN/api/core_api#parametervalue), call `GET /v1/parameter-values`, providing the:
...
You change a Parameter’s value by creating a new [Parameter Value](/vault-core/5-9/EN/api/core_api#parametervalue) for the same owner as the previous value.
...
For more information, see [Creating Parameter Values](/vault-core/5-9/EN/reference/parameters/using_core_api_parameters#creating_parameter_values).
...
-   Cancel a [future-dated](/vault-core/5-9/EN/reference/contracts/contracts_api_4xx/common_examples/generic#future_dated_parameter_values) Account-owned Parameter value
...
If you include `triggers_post_parameter_change_hook` for any `ExpectedParameter` in a CLv4 Smart Contract, you must also define both `triggers_pre_parameter_change_hook` and `triggers_post_parameter_change_hook` (as either `True` or `False`) for ALL of the `expected_parameters` in that CLv4 Smart Contract.
...
You can then create values for this Parameter, either by [creating Parameter Values](/vault-core/5-9/EN/reference/parameters/using_core_api_parameters#creating_parameter_values) or [creating an Account](/vault-core/5-9/EN/reference/accounts/accounts_version_2#creating_an_account) via the v2 Accounts resource.
...
You can then create values for this Parameter, either [via the Parameter Values resource](/vault-core/5-9/EN/reference/parameters/using_core_api_parameters#creating_parameter_values), or when [creating an Account via the v2 Accounts resource](/vault-core/5-9/EN/reference/accounts/accounts_version_2#creating_an_account).
