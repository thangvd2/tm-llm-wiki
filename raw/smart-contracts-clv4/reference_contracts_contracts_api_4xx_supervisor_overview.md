---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_overview"
title: "Supervisor Contracts overview"
scraped_at: "2026-04-20T18:20:00.606Z"
images: 3
---
...
-   The plan Supervisor Contract can supervise that account’s Smart Contract based on its [metadata configuration](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_overview#supervisor_contract_structure)
    
-   The account is not yet supervised by some other plan
    

To fetch existing Supervisor Contracts and create new ones, use the [Core API SupervisorContract](/vault-core/5-8/EN/api/core_api#supervisorcontract) endpoint.
...
![supervision\_entrypoint.png](_assets/supervision_entrypoint.BWxvRRc6_Z1QmwBF_vaultcor.webp)
...
![supervision\_execution\_mode.png](_assets/supervision_execution_mode.mav_5F4F_fqic_vaultcor.webp)
...
![supervisor\_data\_scope.png](_assets/supervisor_data_scope.BT4vGgUi_1jldOR_vaultcor.webp)

## [](#supervisor_contract_structure "Copy link to heading")Supervisor Contract structure

Supervisor Contracts are written in a very similar way to standard Smart Contracts. They define [Hooks](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/concepts#hooks) and supervision associations that enable the supervisor’s hook to be called in addition to, or instead of, the associated supervisees Smart Contract hook. A Supervisor Contract can then inspect data from the Smart Contracts and adjust or create hook directives or return values on behalf of the supervised Smart Contracts. For a list of all hooks that can be defined in a Supervisor Contract, see the [Supervisor Hooks](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_contracts_api_reference4xx/hooks/) section.
...
To override the execution schedule of an `event_type` in a Smart Contract, the Supervisor Contract must declare the following code for the [event\_types](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_contracts_api_reference4xx/metadata#event_types) metadata:
...
For an example of overriding the `scheduled_event_hook`, see [Interest Offsetting example](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_examples#example_usage_of_supervisor_contracts_for_interestrate_offsetting).

### [](#supervision_of_post_posting_hook "Copy link to heading")Supervision of post\_posting\_hook

To override the execution of an account’s `post_posting_hook`, you must specify the `supervise_post_posting_code` in the [supervised\_smart\_contracts](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_contracts_api_reference4xx/metadata#supervised_smart_contracts) metadata. This will trigger the supervisor’s `post_posting_hook` execution for any newly created [PostingInstructionBatch](/vault-core/5-8/EN/api/core_api#postinginstructionbatch) that targets the account.
...
For an example of overriding the `post_posting_hook` hook, see [Rebalancing between accounts example](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_examples#example_usage_of_supervisor_contracts_for_rebalancing).

### [](#supervision_of_pre_posting_hook "Copy link to heading")Supervision of pre\_posting\_hook

To override the execution of an account’s `pre_posting_hook`, you must specify the `pre_posting_hook` [SupervisionExecutionMode](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_types_4xx/enums#supervisionexecutionmode) in the `supervised_hooks` attribute of the [SmartContractDescriptor](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#smartcontractdescriptor) in the [supervised\_smart\_contracts](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_contracts_api_reference4xx/metadata/) metadata.

This will trigger the supervisor’s `pre_posting_hook` execution for any newly created [PostingInstructionBatch](/vault-core/5-8/EN/api/core_api#postinginstructionbatch) that targets the account.

-   INVOKED: The supervisee hook is executed, and the results from the supervisee execution are made available to the supervisor hook via the `vault` method [get\_hook\_results](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/smart_contracts_api_reference4xx/vault#get_hook_result).
...
For an example of invoking the `pre_posting_hook`, see [Overdraft protection example](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_examples#overdraft_protection).
...
-   On Data requirements, see the [Hook Requirements](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/concepts#requirements) concepts section and [Hook Requirements](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_contracts_api_reference4xx/hook_requirements/) Supervisor API section.
    
-   On Optimised data fetching, see [Supervisor data fetchers](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/concepts#supervisor_data_fetchers) concepts section and [Data fetchers](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_contracts_api_reference4xx/account_fetcher_requirements/) Supervisor API section.
...
The structure of a flexible Supervisor Contract is somewhat different to that described in [Supervisor Contract Structure](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/supervisor_overview#supervisor_contract_structure) because:
...
-   Supervision of an event that is part of an [EventTypesGroup](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#eventtypesgroup) will remove it from that group, meaning that the EventTypesGroup must be redefined in the Supervisor Contract.