---
source_url: "https://vault-portal.thoughtmachine.net/vault-core/5-8/EN/reference/accounts/accounts_version_2"
title: "Accounts version 2"
scraped_at: "2026-05-05T20:04:42.455Z"
images: 0
---
...
-   Are using `v2/accounts` and the associated `vault.core_api.v2.accounts.account.events topic` (whether you have integrated anew with these, or have completed [Switching from v1 to v2 Accounts API](/vault-core/5-8/EN/reference/accounts/switching_from_v1_to_v2_accounts_api)).
    
-   Have uploaded a [Contracts Language V4](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/) Smart Contract containing `parameters` or `expected_parameters` syntax (and created any relevant Parameters).
...
Internal Accounts can also be created via the Configuration Layer Utility (CLU) when installing Vault Core. For more information, see [Resources supported for import](/vault-core/5-8/EN/environment_and_installation/infrastructure_docs/configuration_layer_utility_user_guide#resources_supported_for_import) in the CLU guidance.
...
[AccountEvent](/vault-core/5-8/EN/api/core_api#account_events_v2_topic)(AccountCreatedEvent)
...
-   Associate this account with a node in the [Parameter Value Hierarchy](/vault-core/5-8/EN/reference/parameters/building_and_managing_the_parameter_value_hierarchy) by providing a `parameter_value_hierarchy_node_id`
...
-   For more information about Account statuses and lifecycles, see [Account statuses](/vault-core/5-8/EN/reference/accounts/accounts_version_2#account_statuses).
    
-   For more field-level information about Accounts, see the [Core API Accounts version 2](/vault-core/5-8/EN/api/core_api#accounts_version_2) resource.
    
-   For troubleshooting unresolved Account statuses, see [Troubleshooting Customer Accounts](/vault-core/5-8/EN/reference/accounts/accounts_version_2#troubleshooting_customer_accounts).
...
This tutorial assumes that you are aware of the [prerequisites](/vault-core/5-8/EN/reference/accounts/accounts_version_2#before_you_start_2).
...
[AccountEvent](/vault-core/5-8/EN/api/core_api#account_events_v2_topic)
...
If you use the Parameter Value Hierarchy, you can also query the Parameter Values from its point of view. For more information, see [Retrieving effective Parameter values for a node](/vault-core/5-8/EN/reference/parameters/building_and_managing_the_parameter_value_hierarchy#retrieving_effective_parameter_values_for_a_node).
...
![](/vault-core/5-8/EN/_astro/effective_time_multi_scs.EiRyLEVy_Z2lOqpO.svg "Parameter status when spanning multiple Smart Contract Versions")
...
For Vault Core 5, all Account conversions (whether converting to single or multiple destination Smart Contracts) can be performed via multiple calls to `PUT /v2/accounts/{account.id}`.

chat\_bubble

Providing that you have not yet [switched to the Core API Parameters resource](/vault-core/5-8/EN/reference/parameters/switching_to_core_api_parameters), you can still use `POST /v1/account-migrations` and `POST /v1/account-update-batches` for Account conversions. However, `/v2/accounts` is more performant.
...
When a request is made to [OPEN a Customer Account](/vault-core/5-8/EN/reference/accounts/accounts_version_2#creating_an_account) via `POST v2/accounts`, this section explains some typical failures that can occur, and how to remedy them:
...
-   Call PUT `v2/accounts` to [update the Account(s) to OPEN](/vault-core/5-8/EN/reference/accounts/accounts_version_2#opening_an_account)
...
-   Instruct new Postings via the [Postings APIs](/vault-core/5-8/EN/api/postings_api#postings_apis)
    
-   Depending on the scenario, you may want to remedy the failed Account(s) via a call to PUT `v2/accounts` to [Convert the Account(s)](/vault-core/5-8/EN/reference/accounts/accounts_version_2#account_conversions) to a new Smart Contract.
...
-   It is likely that [Postings](/vault-core/5-8/EN/api/postings_api#postings_apis) or [AccountNotificationDirectives](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#accountnotificationdirective) have failed. Remedy these as applicable.
...
When a request is made to [close a Customer Account](/vault-core/5-8/EN/reference/accounts/accounts_version_2#closing_a_customer_account) via POST `v2/accounts`, this section explains some failures that can occur, and how to remedy them:
...
-   Typically the balance failed to reach zero, or some schedules failed to terminate. Make the necessary changes (for example by instructing [Postings](/vault-core/5-8/EN/api/postings_api#using_the_api) to zero the balance)
    
-   Call `PUT /v2/accounts{account.id}` to resend the [close account request](/vault-core/5-8/EN/reference/accounts/accounts_version_2#closing_a_customer_account).
...
-   This is most likely due to an [AccountNotificationDirective](/vault-core/5-8/EN/reference/contracts/contracts_api_4xx/common_types_4xx/classes#accountnotificationdirective) being too large for Kafka to consume. In this case, the Smart Contract will need remediation.
...
![Vault\_Core\_5\_0\_\_diagrams\_-\_Account\_status\_overview.svg](/vault-core/5-8/EN/_astro/uuid-fbd1938d-38a8-8b15-cd45-ebdc6a3c19e6-en.CdxHQ1qQ_Z15WiUB.webp)
...
For more information, see [Troubleshooting Customer Accounts](/vault-core/5-8/EN/reference/accounts/accounts_version_2#troubleshooting_customer_accounts).
...
A *persistent* status is one that an account can be in indefinitely, with guarantees relating to whether Smart Contract Hooks have run. You can request any persistent status via the [Accounts v2](/vault-core/5-8/EN/api/core_api#account) resource:
...
Transitory statuses cannot be requested, and cannot provide guarantees about the state of the account; some, none, or all directives returned by a Smart Contract Hook may have been committed. While in transitory statuses, certain operations are prevented to avoid invalid conditions. For more information, see the Core API [Accounts v2](/vault-core/5-8/EN/api/core_api#v2) resource.
